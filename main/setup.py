...
cppFiles = [
            ...
            'src/Graphics/characterBitmap.cpp',
...
if not config["minimalCppFiles"]:
    cppFiles += [
            ...
            'src/Objects/CMarkerBodyBeamShape.cpp',
            ...
            'src/Objects/CObjectJointSliding.cpp',
...
def load_converter():
    ...
    module_path = project_root / "src" / "pythonGenerator" / "autoGenerateDocstrings.py"
    if not module_path.exists():
        raise FileNotFoundError(f"Converter not found at {module_path}")
    spec = importlib.util.spec_from_file_location("autoGenerateDocstrings", module_path)
    mod = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    spec.loader.exec_module(mod)
    return mod.ConvertExuDoc2Docstrings

def TreeConvert2Temp(sourcePath: Path, destPath: Path):
    print('------- Exudyn: convert to docstrings -----------')
    
    #does not work here:
    #from src.pythonGenerator.autoGenerateDocstrings import ConvertExuDoc2Docstrings

    ConvertExuDoc2Docstrings=load_converter()
    
    # remove existing files->could contain invalid files!
    if destPath.exists() and 'tempExudyn' in str(destPath): #double check for destPath ...
        shutil.rmtree(destPath)

    shutil.copytree(sourcePath, destPath)

    for p in destPath.rglob("*.py"):
        if not (
                'GUI.py' in p.name
                or 'resultsMonitor.py' in p.name
                or '__init__.py' in p.name
                or 'itemInterface.py' in p.name
                ):
            #print('convert to docstrings in file: ', p.name)
            # txt = p.read_text(encoding="utf-8")
            # p.write_text(txt, encoding="utf-8")
            try:
                ConvertExuDoc2Docstrings(p, p)
            except:
                print('problem with Docstring:',p.name)

    print('------- Exudyn: convert to docstrings finished -----------')




#convert exudyn python files (docstrings, etc.)
class BuildPy(_build_py):
    def run(self):
        ...
        TreeConvert2Temp(src_root/'exudyn', tmp_root/'exudyn')

        self.package_dir = {"": str(tmp_root)}
        self.packages = find_namespace_packages(
            where=str(tmp_root),
            include=("exudyn", "exudyn.robotics"),
        )
        
        # dist = self.distribution
        # old_pkg_dir = dict(dist.package_dir or {})
        # old_packages = list(dist.packages or [])
        # try:
        #     dist.package_dir = {"": str(tmp_root)}
        #     dist.packages = find_namespace_packages(
        #         where=str(tmp_root),
        #         include=("exudyn", "exudyn.robotics"),
        #     )
        #     super().run()
        # finally:
        #     dist.package_dir = old_pkg_dir
        #     dist.packages = old_packages    

        print('------- Exudyn: finished BuildPy -----------')

        # Proceed with the normal build step using the overridden options
        super().run()
...
#    A custom build extension for adding compiler-specific options.
    ...
    c_opts = {
        ...
        'unix': [
         '-O3',  #WORKS #full optimization, overrides -O2 standard; improves 50% on laptop i7-1370P; slower compilation
         ...
         ##'-fno-math-errno',  #math does not raise errors
         ##'-fno-trapping-math',#
         ##'-freciprocal-math', #can use a*(1/b) instead of a/b
         ##'-fno-signed-zeros', #no difference between +0 and -0
         ##'-fassociative-math',#change (a+b)+c into a+(b+c), a*(b*c) ...
         #'-march=x86-64-v3', #untested #portable baseline for AVX2+FMA
         #'-march=x86-64-v4', #works but untested unter porting #no speedup on AMD #portable baseline for AVX512
         #++++++++++++++++++++++++++++++
...
#		'-Wall',
    ...
    if sys.platform == 'darwin':
        ...
        c_opts['unix'].remove('-Wno-template-id-cdtor') #does not work with MacOS compiler; 2026-08-05
        ...
        '-Wno-unused-but-set-variable', #since 2026 for Eigen, etc.
        ]+commonCopts
