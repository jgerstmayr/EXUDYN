...
BATCH_MODE = False
BATCH_PARAMS = {}
SIMULATION_MODE = 'accuracy'  # 'stiffness' 或 'accuracy'
...
if len(sys.argv) > 1:
    ...
    if os.path.exists(param_file):
        ...
        BATCH_MODE = True
        SIMULATION_MODE = BATCH_PARAMS.get('simulation_mode', 'accuracy')
        print(f"[BATCH MODE] 从文件读取参数: {param_file}")
        print(f"[BATCH MODE] 仿真模式: {SIMULATION_MODE}")

# 默认参数（会被apply_batch_params覆盖）
eccentric_error = 0  # 偏心圆半径误差
eccentricity_error = 0  # 偏心距误差
angle_error = 0  # 偏心角误差
bearing_clearance_adjust = 0  # 轴承间隙调整
input_speed_rpm = 60  # 输入转速 (rpm)
load_torque = 100  # 负载扭矩 (N·m)
simulation_time = 1.0  # 仿真时间 (s)
output_dir = './solution'

# 刚度和精度测试扭矩（必须在apply_batch_params之前定义）
STIFFNESS_TEST_TORQUE = 3200.0  # N·m（大扭矩，用于刚度测试）
ACCURACY_TEST_TORQUE = load_torque  # N·m（工作扭矩，用于精度测试）

def apply_batch_params():
    """从BATCH_PARAMS应用参数到全局变量"""
    global eccentric_error, eccentricity_error, angle_error
    global bearing_clearance_adjust, input_speed_rpm, load_torque
    global simulation_time, useGraphics, output_dir
    global e, r_crank_eccentric, bearing_clearance
    global omega_input_target, outputTorqueZ, tEnd, SIMULATION_MODE, USE_CYCLIC_TORQUE
    global ANGLE_OFFSET  # 应用角度误差
    global STIFFNESS_TEST_TORQUE, ACCURACY_TEST_TORQUE  # 刚度和精度的测试扭矩
    # 新增：每个曲柄轴的随机化参数列表
    global e_list, r_crank_eccentric_list, angle_offset_list
    global r_needle_pitch_list, r_hole_list
    ...
    if BATCH_MODE:
        ...
        # 工况参数
        input_speed_rpm = BATCH_PARAMS.get('input_speed', 60)
        load_torque = BATCH_PARAMS.get('load_torque', 100)
        simulation_time = BATCH_PARAMS.get('simulation_time', 1.0)
        SIMULATION_MODE = BATCH_PARAMS.get('simulation_mode', 'accuracy')

        # 输出目录
        output_dir = BATCH_PARAMS.get('output_dir', './results')

        # 图形显示（批量模式默认关闭，可通过参数开启）
        global useGraphics
        ...
        # 计算每个曲柄的具体参数
        e_list = 2.2e-3 + err_e_list
        r_crank_eccentric_list = 22.5e-3 + err_r_list
        ...
        # 根据模式调整参数
        if SIMULATION_MODE == 'stiffness':
            # 刚度计算模式：准静态，扭矩循环加载
            omega_input_target = 0  # 输入轴不转
            outputTorqueZ = STIFFNESS_TEST_TORQUE  # 使用大扭矩（3200 N·m）
            USE_CYCLIC_TORQUE = True  # 使用循环扭矩
            simulation_time = 2.5  # 完整扭矩循环
        else:  # 'accuracy'
            # 精度计算模式：动态驱动，恒定扭矩
            omega_input_target = input_speed_rpm * 2 * np.pi / 60
            outputTorqueZ = ACCURACY_TEST_TORQUE  # 使用工作扭矩（可配置）
            USE_CYCLIC_TORQUE = False  # 使用恒定扭矩
    else:
        # 非批量模式，使用默认值（全部一致）
        e_list = np.full(n_cranks, 2.2e-3)
        r_crank_eccentric_list = np.full(n_cranks, 22.5e-3)
...
# 法兰约束模式：True=使用ACBB角接触球轴承，False=使用简单GenericJoint约束
# 精度模式建议False以简化模型，刚度模式需要True以体现轴承刚度
USE_ACBB_BEARING = False  # 设为False：法兰只约束除Z轴旋转外的5个自由度

useGraphics = True  # 启用图形窗口显示（批量模式自动关闭）
...
z_b = 40
z_g = z_b - 1
e = 2.2e-3  # 偏心距 (m) - 基准值，实际值由 apply_batch_params() 设置
R_z = 119.5e-3  # 针齿分布圆半径 (m)
r_z = 5e-3  # 针齿半径 (m)

r_crank_main = 20e-3  # 曲柄主轴半径 (m)
r_crank_eccentric = 22.5e-3  # 曲柄偏心段半径 (m) - 基准值，实际值由 apply_batch_params() 设置
...
bearing_clearance = 0.005e-3  # 轴承径向间隙 (m) - 基准值，实际值由 apply_batch_params() 设置
r_needle_pitch = r_crank_eccentric + r_needle + bearing_clearance  # 滚针节圆半径 (m) - 会被覆盖
r_hole = r_needle_pitch + r_needle + bearing_clearance  # 摆线轮孔半径 (m) - 会被覆盖
...
# 接触摩擦系数（库仑摩擦）
# 接触摩擦系数（库仑摩擦）
if SIMULATION_MODE == 'accuracy':
    # 精度计算模式：假设无摩擦（或用户指定的其他情况）
    friction_cycloid_pin = 0.0
    friction_crank_needle = 0.05
    friction_needle_cycloid_hole = 0.01
    friction_flange_bearing = 0.06
    friction_pin_shell = 0.06
    print("[INFO] Accuracy mode: Friction coefficients set to 0.0")
else:
    # 默认/刚度模式：使用标准摩擦系数
    friction_cycloid_pin = 0.01            # 摆线齿廓-针齿
...
if BATCH_MODE:
    print(f"[BATCH MODE] 已应用参数")
    print(f"  输入转速: {input_speed_rpm} rpm")
    print(f"  负载扭矩: {load_torque} N·m")
    print(f"  仿真时间: {simulation_time} s")
    print("=" * 60)
...
# =================== 驱动参数（刚度和精度分析） ===================
# 目标输入转速（rad/s）- 从input_speed_rpm转换而来
# 注意：这里设置的值会被apply_batch_params()覆盖
omega_input_target = input_speed_rpm * 2 * np.pi / 60

# 刚度测试扭矩（大扭矩，用于绘制滞回曲线）
STIFFNESS_TEST_TORQUE = 3200.0  # N·m（与原cycloid.py一致）
# 精度测试扭矩（工作扭矩）
ACCURACY_TEST_TORQUE = load_torque  # N·m（可配置，默认100）

outputTorqueZ = ACCURACY_TEST_TORQUE  # 默认值，会被apply_batch_params覆盖

torqueSegmentTime = 0.5  # 每段持续时间；默认序列 0→+max→0→-max→0→+max→0

TORQUE_SEQUENCE_DEFAULT = [0.0, 1.0, 0.0, -1.0, 0.0, 1.0]  # 完整循环：最后保持满载

# 扭矩加载模式标志
USE_CYCLIC_TORQUE = True  # True=循环加载(刚度), False=恒定扭矩(精度)
...
# =================== 创建三个曲柄轴 ===================
# 注意：每个曲柄轴的参数可能不同，所以在循环内部创建 crank_segments
...
if USE_FLEXIBLE_CRANKSHAFT:
    ...
    for i_crank in range(n_cranks):
        ...
        angle_crank = angle_offset_list[i_crank] + 2 * np.pi * i_crank / n_cranks  # 120度均布 + 随机角度误差
    ...
    for i_crank in range(n_cranks):
        ...
        mbs.AddObject(GenericJoint(
            ...
            constrainedAxes=[0, 0, 1, 1, 1, 0],  # 释放XY平移，约束Z平移和XY倾转，释放Z旋转
            visualization=VObjectJointGeneric(axesRadius=0.5e-3, axesLength=1.0e-3)
        ))
        ...
        # 使用当前曲柄的偏心距
        current_eccentric_offset = e_list[i_crank]
        mEcc1 = mbs.AddMarker(MarkerBodyRigid(bodyNumber=oCrank, 
                                               localPosition=[0, current_eccentric_offset, z_ecc1_center - z_center]))
        crankshaft_markers_ecc1.append(mEcc1)
        
        mEcc2 = mbs.AddMarker(MarkerBodyRigid(bodyNumber=oCrank, 
                                               localPosition=[0, -current_eccentric_offset, z_ecc2_center - z_center]))
        crankshaft_markers_ecc2.append(mEcc2)
...
if USE_ACBB_BEARING:
    ...
    # 使用简单GenericJoint约束：约束除Z轴旋转外的5个自由度
    print("\n" + "="*60)
    print("Creating simple GenericJoint constraints for flanges...")
    print("  (5 DOF constrained, only Z-rotation free)")
    print("="*60)
    
    # 输入法兰约束：[X, Y, Z, Rx, Ry, Rz] - Rz(=5)自由，其他约束
    mbs.AddObject(GenericJoint(
        markerNumbers=[mGroundInputFlange, mInputFlange],
        constrainedAxes=[1, 1, 1, 1, 1, 0],  # Rz不约束
        visualization=VObjectJointGeneric(show=False)
    ))
    print("  Input flange: 5 DOF constrained, Rz free")
    
    # 输出法兰约束：[X, Y, Z, Rx, Ry, Rz] - Rz(=5)自由，其他约束
    mbs.AddObject(GenericJoint(
        markerNumbers=[mGroundOutputFlange, mOutputFlangeMarker],
        constrainedAxes=[1, 1, 1, 1, 1, 0],  # Rz不约束
        visualization=VObjectJointGeneric(show=False)
    ))
    print("  Output flange: 5 DOF constrained, Rz free")
    print("="*60)
...
if USE_ACBB_BEARING:
    ...
    print("  Flange coupling: 6DOF RigidBodySpringDamper")
else:
    # 简单约束模式：仅使用扭转弹簧（Rz方向）
    # 其他5个DOF已通过GenericJoint刚性约束
    flange_torsion_stiffness = 1e10  # 扭转刚度 (N·m/rad)
    flange_torsion_damping = 1e6     # 扭转阻尼 (N·m·s/rad)
    
    mbs.AddObject(ObjectConnectorTorsionalSpringDamper(
        markerNumbers=[mInputFlangeSpring, mOutputFlangeSpring],
        stiffness=flange_torsion_stiffness,
        damping=flange_torsion_damping,
        rotationMarker0=np.eye(3),
        rotationMarker1=np.eye(3),
        visualization=VObjectConnectorTorsionalSpringDamper(show=True, drawSize=5e-3)
    ))
    print(f"  Flange coupling: TorsionalSpringDamper (k={flange_torsion_stiffness:.0e} N·m/rad)")

# 输出法兰扭矩控制
mOutputFlangeLoad = mbs.AddMarker(MarkerBodyRigid(bodyNumber=oOutputFlange, localPosition=[0, 0, 0]))

# 根据扭矩加载模式标志选择加载方式
# USE_CYCLIC_TORQUE=True: 循环扭矩（刚度模式）
# USE_CYCLIC_TORQUE=False: 恒定扭矩（精度模式）
if USE_CYCLIC_TORQUE:
    # 刚度模式：扭矩循环加载（0→+max→0→-max→0）
    def output_torque_user_function(mbs, t, loadVector):
        return [0.0, 0.0, torque_multi_step(outputTorqueZ, t)]
else:
    # 精度模式：扭矩斜坡加载（0→max），与输入速度同步
    TORQUE_RAMP_TIME = 0.2  # 与输入速度加速时间一致
    def output_torque_user_function(mbs, t, loadVector):
        if t <= 0.0:
            torque_ratio = 0.0
        elif t < TORQUE_RAMP_TIME:
            torque_ratio = t / TORQUE_RAMP_TIME  # 线性斜坡
        else:
            torque_ratio = 1.0  # 满载
        return [0.0, 0.0, -outputTorqueZ * torque_ratio]
...
for i_crank in range(n_cranks):
    ...
    for i_needle in range(n_needles):
        ...
        mbs.AddObject(GenericJoint(markerNumbers=[mCycloid1CageHole, mNeedle1],
                                   constrainedAxes=[1, 1, 1, 1, 1, 1],  # 锁定所有DOF包括Rz
...
_last_progress_percent = 0  # 上次打印的进度百分比
...
def _post_step_contact_logger(mbs, step_information):
    ...
    # 进度输出（每10%打印一次）
    if tEnd > 0:
        progress_percent = int(current_time / tEnd * 100)
        if progress_percent >= _last_progress_percent + 10:
            _last_progress_percent = (progress_percent // 10) * 10
            print(f"  仿真进度: {_last_progress_percent}% (t={current_time:.4f}s / {tEnd:.2f}s)", flush=True)
...
stepSize = 2e-4  # 设置初始步长
tEnd = simulation_time  # 仿真时长 - 使用批量参数（传动精度分析需要足够长时间）
...
simulationSettings.solutionSettings.recordImagesInterval = -1  # 不录制图像
...
if useGraphics:
    ...
    exu.StartRenderer()  # 使用全局启动方式，仿真自动开始
...
# =================== 传动精度计算（批量模式） ===================
# 总传动比 = 输入到曲柄 × 曲柄到输出 = 3.0 × 39.0 = 117.0
TOTAL_REDUCTION_RATIO = 117.0

def calculate_transmission_accuracy(data_input_rot, data_output_rot, reduction_ratio=TOTAL_REDUCTION_RATIO):
    """
    计算传动精度指标

    Args:
        data_input_rot: 输入轴旋转矩阵数据 [time, R11, R12, ..., R33]
        data_output_rot: 输出轴旋转矩阵数据 [time, R11, R12, ..., R33]
        reduction_ratio: 总传动比（默认117.0 = 3.0 × 39.0）

    Returns:
        accuracy_metrics: 字典，包含各项精度指标
    """
    ...
    time_input = data_input_rot[:, 0]
    time_output = data_output_rot[:, 0]

    angle_input = extract_z_angle(data_input_rot)
    angle_output = extract_z_angle(data_output_rot)

    # 插值到同一时间点
    time_common = np.linspace(max(time_input[0], time_output[0]),
                              min(time_input[-1], time_output[-1]),
                              min(len(time_input), len(time_output)))

    angle_input_interp = np.interp(time_common, time_input, angle_input)
    angle_output_interp = np.interp(time_common, time_output, angle_output)

    # ======== 计算传动误差 =========
    # 理论输出角度 = 输入角度 / 减速比
    theoretical_output = angle_input_interp / reduction_ratio
    transmission_error = angle_output_interp - theoretical_output

    # 转换为角秒（arcseconds）
    transmission_error_arcsec = transmission_error * (180/np.pi) * 3600

    # ======== 统计指标 ========
    # 去除前20%瞬态数据
    stable_start = int(len(transmission_error_arcsec) * 0.2)
    stable_error = transmission_error_arcsec[stable_start:]

    metrics = {
        'mean_transmission_error_arcsec': float(np.mean(stable_error)),
        'transmission_error_std_arcsec': float(np.std(stable_error)),
        'transmission_error_peak_to_peak_arcsec': float(np.max(stable_error) - np.min(stable_error)),
        'max_instantaneous_error_arcsec': float(np.max(np.abs(stable_error))),
        'lost_motion_arcmin': float((np.max(stable_error) - np.min(stable_error)) / 60),  # 转为角分
        'actual_reduction_ratio': float(np.mean(angle_output_interp) / np.mean(angle_input_interp)) if np.mean(angle_input_interp) != 0 else 0,
    }

    return metrics, time_common, transmission_error_arcsec

def calculate_stiffness(data_input_rot, data_output_rot):
    """
    计算扭转刚度和迟滞特性（刚度分析模式）

    Args:
        data_input_rot: 输入轴旋转矩阵数据
        data_output_rot: 输出轴旋转矩阵数据

    Returns:
        stiffness_metrics: 字典，包含刚度指标
    """
    if data_input_rot is None or data_output_rot is None:
        print("警告：缺少输入或输出旋转数据")
        return {}
    ...
    # 输出轴角度转换为arcmin（与文献2一致）
    angle_output_arcmin = angle_output_interp * (180/np.pi) * 60

    # 对应的扭矩值（使用扭矩序列）
    torque_values = np.array([torque_multi_step(outputTorqueZ, float(t)) for t in time_common])
    ...
    if np.sum(loading_mask) > 10:
        ...
        stiffness_slope = 1.0 / coeffs[0] if abs(coeffs[0]) > 1e-10 else np.nan  # N·m/arcmin
    ...
    # ======== 计算回程间隙 =========
    angle_range = np.max(angle_output_arcmin) - np.min(angle_output_arcmin)
    lost_motion = angle_range  # arcmin

    # ======== 计算迟滞面积（能量损耗）============
    # 兼容新旧版本scipy (simps在1.12+被重命名为simpson)
    try:
        from scipy.integrate import simpson as simps_func
    except ImportError:
        from scipy.integrate import simps as simps_func
    if len(time_common) > 10:
        # 排序数据以计算迟滞曲线面积
        sorted_idx = np.argsort(torque_values)
        torque_sorted = torque_values[sorted_idx]
        angle_sorted = angle_output_arcmin[sorted_idx]
        hysteresis_area = abs(simps_func(angle_sorted, x=torque_sorted))  # N·m·arcmin
    else:
        hysteresis_area = np.nan

    metrics = {
        'torsional_stiffness_Nm_per_arcmin': float(stiffness_slope),
        'stiffness_r_squared': float(r_squared) if not np.isnan(r_squared) else np.nan,
        'lost_motion_arcmin': float(lost_motion),
        'hysteresis_area': float(hysteresis_area) if not np.isnan(hysteresis_area) else np.nan,
        'max_angle_arcmin': float(np.max(angle_output_arcmin)),
        'min_angle_arcmin': float(np.min(angle_output_arcmin)),
    }

    return metrics, time_common, angle_output_arcmin, torque_values
...
if BATCH_MODE:
    ...
    if SIMULATION_MODE == 'stiffness':
        # 刚度计算模式
        stiffness_metrics, time_common, angle_output, torque_values = calculate_stiffness(
        ...
        if stiffness_metrics:
            ...
            # 保存原始数据
            np.savetxt(f'{output_dir}/input_rotation.txt', data_input_rot,
                       header='time R11 R12 R13 R21 R22 R23 R31 R32 R33', comments='')
            np.savetxt(f'{output_dir}/output_rotation.txt', data_output_rot,
                       header='time R11 R12 R13 R21 R22 R23 R31 R32 R33', comments='')
            ...
            with open(f'{output_dir}/stiffness_metrics.txt', 'w', encoding='utf-8') as f:
                ...
                f.write(f"  偏心角误差: {np.rad2deg(angle_error):.2f} deg\n")
                f.write(f"  轴承间隙调整: {bearing_clearance_adjust*1e6:.2f} μm\n")
                f.write(f"  负载扭矩: {load_torque} N·m\n")
                f.write(f"\n刚度指标:\n")
        ...
        # 精度计算模式
        accuracy_metrics, time_common, transmission_error = calculate_transmission_accuracy(
            data_input_rot, data_output_rot
        )

        if accuracy_metrics:
            print("\n传动精度结果:")
            print(f"  平均传动误差: {accuracy_metrics['mean_transmission_error_arcsec']:.2f} arcsec")
            print(f"  传动误差标准差: {accuracy_metrics['transmission_error_std_arcsec']:.2f} arcsec")
            print(f"  传动误差峰峰值: {accuracy_metrics['transmission_error_peak_to_peak_arcsec']:.2f} arcsec")
            print(f"  瞬时最大误差: {accuracy_metrics['max_instantaneous_error_arcsec']:.2f} arcsec")
            print(f"  回程间隙: {accuracy_metrics['lost_motion_arcmin']:.4f} arcmin")
            print(f"  实际减速比: {accuracy_metrics['actual_reduction_ratio']:.6f}")
            ...
            # 保存传动误差曲线
            error_data = np.column_stack([time_common, transmission_error])
            np.savetxt(f'{output_dir}/transmission_error_arcsec.txt', error_data,
                       header='time error_arcsec', comments='')

            # 保存精度指标（文本格式）
            with open(f'{output_dir}/accuracy_metrics.txt', 'w', encoding='utf-8') as f:
                f.write("RV减速器传动精度分析结果\n")
                f.write("="*50 + "\n")
                f.write(f"参数设置:\n")
                f.write(f"  偏心圆半径误差: {eccentric_error*1e6:.2f} μm\n")
                f.write(f"  偏心距误差: {eccentricity_error*1e6:.2f} μm\n")
                f.write(f"  偏心角误差: {np.rad2deg(angle_error):.2f} deg\n")
                f.write(f"  轴承间隙调整: {bearing_clearance_adjust*1e6:.2f} μm\n")
                f.write(f"  输入转速: {input_speed_rpm} rpm\n")
                f.write(f"  负载扭矩: {load_torque} N·m\n")
                f.write(f"  仿真时间: {simulation_time} s\n")
                f.write(f"\n传动精度指标:\n")
                f.write(f"  平均传动误差: {accuracy_metrics['mean_transmission_error_arcsec']:.2f} arcsec\n")
                f.write(f"  传动误差标准差: {accuracy_metrics['transmission_error_std_arcsec']:.2f} arcsec\n")
                f.write(f"  传动误差峰峰值: {accuracy_metrics['transmission_error_peak_to_peak_arcsec']:.2f} arcsec\n")
                f.write(f"  瞬时最大误差: {accuracy_metrics['max_instantaneous_error_arcsec']:.2f} arcsec\n")
                f.write(f"  回程间隙: {accuracy_metrics['lost_motion_arcmin']:.4f} arcmin\n")
                f.write(f"  实际减速比: {accuracy_metrics['actual_reduction_ratio']:.6f}\n")
...
if BATCH_MODE:
    # 根据模式检查对应的指标
    if SIMULATION_MODE == 'stiffness':
        sys.exit(0 if stiffness_metrics else 1)
    else:
        sys.exit(0 if accuracy_metrics else 1)
