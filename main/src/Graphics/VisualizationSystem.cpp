...
void VisualizationSystem::UpdateGraphicsData(VisualizationSystemContainer& visualizationSystemContainer)
{
	...
	try
	{
		...
		{
			RenderViewDataVSC& RVD = visualizationSystemContainer.GetRenderViewData(VisualizationSystemContainer::mainViewID);
			RVD.saveImage = true;
			RVD.saveImageOpenGL = true; //send immediately signal to openGL that frame needs to be recorded
			...
			|| visualizationSystemContainer.UpdateGraphicsDataNowInternal()) && systemData->GetCData().IsSystemConsistent())
		{
			...
			if (visualizationSystemContainer.settings.contour.outputVariable != OutputVariableType::_None)
			{
				...
				for (auto& item : graphicsData.glLines)
				{
					...
					{
						item.color1 = visualizationSystemContainer.ColorBarColor(minVal, maxVal, item.color1[0], alphaTransparency);
						item.color2 = visualizationSystemContainer.ColorBarColor(minVal, maxVal, item.color2[0], alphaTransparency);
					}
				}
				for (auto& item : graphicsData.glCirclesXY)
				{
					if (item.color[3] == contourPlotFlag)
					{
						item.color = visualizationSystemContainer.ColorBarColor(minVal, maxVal, item.color[0], alphaTransparency);
					}
				}
				for (auto& item : graphicsData.glSpheres)
				{
