...
//delete: #include "Autogenerated/CMarkerGenericBodyPosition.h"
...
void VisualizationObjectSuperElement::UpdateGraphics(const VisualizationSettings& visualizationSettings, VisualizationSystem* vSystem, Index itemNumber)
{
	...
	CObjectFFRF* cObject = (CObjectFFRF*)vSystem->systemData->GetCObjects()[itemNumber];

	Matrix3D refRot = ((const CNodeRigidBody*)cObject->GetCNode(CObjectFFRF::rigidBodyNodeNumber))->GetRotationMatrix(ConfigurationType::Visualization);
	Vector3D refPos = ((const CNodeRigidBody*)cObject->GetCNode(CObjectFFRF::rigidBodyNodeNumber))->GetPosition(ConfigurationType::Visualization);
...
//delete:
////! Update visualizationSystem -> graphicsData for item
//void VisualizationMarkerGenericBodyPosition::UpdateGraphics(const VisualizationSettings& visualizationSettings, VisualizationSystem* vSystem, Index itemNumber)
//{
//	Float4 currentColor = visualizationSettings.markers.defaultColor;
//
//	CMarkerGenericBodyPosition* cMarker = (CMarkerGenericBodyPosition*)vSystem->systemData->GetCMarkers()[itemNumber];
//	CObjectBody* cBody = &vSystem->systemData->GetCObjectBody(cMarker->GetObjectNumber());
//
//	Vector3D pos; //global marker position
//	cMarker->GetPosition(*vSystem->systemData, pos, ConfigurationType::Visualization);
...
//			A  = ((const CNodeRigidBody&)vSystem->systemData->GetCNode(cBody->GetNodeNumber(CObjectGenericODE2::rigidBodyNodeNumber))).GetRotationMatrix(ConfigurationType::Visualization);
//			p0 = ((const CNodeRigidBody&)vSystem->systemData->GetCNode(cBody->GetNodeNumber(CObjectGenericODE2::rigidBodyNodeNumber))).GetPosition(ConfigurationType::Visualization);
//		}
//
//		Float4 alternativeColor = EXUvis::ModifyColor(currentColor, 0.25f);
//		for (Index node: cMarker->GetParameters().nodeNumbers)
//		{
//			Vector3D p(((const CNodeODE2&)vSystem->systemData->GetCNode(cBody->GetNodeNumber(node))).GetPosition(ConfigurationType::Visualization));
//			if (cMarker->GetParameters().useFirstNodeAsReferenceFrame)
//			{
//				p = p0 + A * p;
//			}
