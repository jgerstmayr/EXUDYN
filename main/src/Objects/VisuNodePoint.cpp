...
#include "Autogenerated/VisuObjectCustomContact.h" //for CustomContact visualization
#include "Autogenerated/VisuObjectContactFewTeeth.h"
#include "Autogenerated/VisuObjectContactExternalInvolute.h"

class CObjectContactFewTeeth; // forward declaration
class CObjectContactExternalInvolute; // forward declaration
	...
	TCObject* cObject, TVisuObject* vObject)
{
	...
	if (!hasGeometry)
	{
		...
		for (Index i = 0; i <= tiling; i++)
		{
			...
			// MODIFIED: For Z-axis beam, use Z coordinate for beam axis
			Vector3D pLoc({ 0., 0., x });
	...
	else
	{
		...
		for (Index i = 0; i <= tiling; i++)
		{
			...
			if (i != 0)
			{
				// MODIFIED: For Z-axis beam, cross-section is in X-Y plane instead of Y-Z plane
				Vector3D p0loc({ pg.Last().X(), pg.Last().Y(), 0. });
				for (Index j = 0; j < pg.NumberOfItems(); j++)
				{
					Vector3D p1loc({ pg[j].X(), pg[j].Y(), 0. });
...
void VisualizationObjectContactCurveCircles::UpdateGraphics(const VisualizationSettings& visualizationSettings, VisualizationSystem* vSystem, Index itemNumber)
{
	...
	const bool drawForces = visualizationSettings.contact.showContactForces;
	const bool drawValues = visualizationSettings.contact.showContactForcesValues;
	const Real forceScale = visualizationSettings.contact.contactForcesFactor;
	const Vector& segForceX = cItem->GetSegmentsForceLocalX();
	const Vector& segForceY = cItem->GetSegmentsForceLocalY();
	constexpr bool drawSegments = false;
	...
	if (visualizationSettings.connectors.showContact)
	{
		for (Index jSeg = 0; jSeg < nSegments; jSeg++)
		{
			...
			Vector3D lastForceTip({ 0.,0.,0. });
			bool lastForceValid = false;
			if (drawForces && data[jSeg * CObjectContactCurveCircles::nDataVariablesPerSegment + CObjectContactCurveCircles::dataIndexCircle] >= 0
				&& segForceX.NumberOfItems() == nSegments && segForceY.NumberOfItems() == nSegments)
			{
				Vector2D p0Local({ segData(jSeg,0), segData(jSeg,1) });
				Vector2D p1Local({ segData(jSeg,2), segData(jSeg,3) });
				Vector2D midLocal = 0.5 * (p0Local + p1Local);
				Vector3D midGlobal = refPos + refRot * Vector3D({ midLocal[0], midLocal[1], 0 });
				Vector2D forceLocal({ -segForceX[jSeg], -segForceY[jSeg] });
				Real forceNorm = forceLocal.GetL2Norm();
				if (forceNorm > 0.)
				{
					Vector3D forceGlobal = refRot * Vector3D({ forceLocal[0], forceLocal[1], 0 });
					Vector3D forceTip = midGlobal + forceScale * forceGlobal;
					vSystem->graphicsData.AddLine(midGlobal, forceTip, forceColor, forceColor, itemID);
					if (lastForceValid)
					{
						vSystem->graphicsData.AddLine(lastForceTip, forceTip, forceColor, forceColor, itemID);
					}
					lastForceTip = forceTip;
					lastForceValid = true;

					if (drawValues)
					{
						STDstring text = EXUstd::Num2String(forceNorm, visualizationSettings.general.rendererPrecision);
						vSystem->graphicsData.AddText(forceTip, forceColor, text, 0.f, 0.f, 0.f, itemID);
					}
				}
			}
...
//! AUTO:  update graphics from given GraphicsData and VisualizationSettings - CustomContact
void VisualizationObjectCustomContact::UpdateGraphics(const VisualizationSettings& visualizationSettings, VisualizationSystem* vSystem, Index itemNumber)
{
	// Simple visualization - no graphics drawn for this connector
	// Could potentially draw a line between contact point and segment in future
}

//! AUTO:  update graphics from given GraphicsData and VisualizationSettings - ContactCircleCircle
void VisualizationObjectContactCircleCircle::UpdateGraphics(const VisualizationSettings& visualizationSettings, VisualizationSystem* vSystem, Index itemNumber)
{
	Index itemID = Index2ItemID(itemNumber, ItemType::Object, vSystem->GetSystemID());
	Float4 currentColor = visualizationSettings.connectors.defaultColor;
	...
	CObjectContactCircleCircle* cItem = (CObjectContactCircleCircle*)vSystem->systemData->GetCObjects()[itemNumber];
	...
	if (visualizationSettings.contact.showSpheres)
	{
		Float4 sphereColor = visualizationSettings.contact.colorSpheres;
		if (color[0] != -1.f) { sphereColor = color; }

		// Draw circles as rings in XY plane
		Real r1 = cItem->GetParameters().radius1;
		Real r2 = cItem->GetParameters().radius2;
		
		// Identity rotation matrix (circles in XY plane)
		Matrix3D rot1(3, 3);
		for (Index i = 0; i < 3; ++i) {
			for (Index j = 0; j < 3; ++j) {
				rot1(i, j) = 0.;
		...
		if (absR1 > 0.) {
			EXUvis::DrawCircle(pos[0], rot1, absR1, sphereColor, 
				vSystem->graphicsData, itemID, visualizationSettings.general.circleTiling);
		}
		if (absR2 > 0.) {
			EXUvis::DrawCircle(pos[1], rot1, absR2, sphereColor, 
				vSystem->graphicsData, itemID, visualizationSettings.general.circleTiling);
		}
	}
	...
	if (drawForces)
	{
		const Vector& visData = cItem->GetCNode(0)->GetVisualizationCoordinateVector();
		if (visData.NumberOfItems() >= 3)
		{
			Vector3D midPoint = 0.5 * (pos[0] + pos[1]);
			Vector3D normalDir = (pos[0] - pos[1]);
			Real norm = normalDir.GetL2Norm();
			if (norm > 0.)
			{
				normalDir *= 1. / norm;
				Real fNormal = visData[2];
				Vector3D forceTip = midPoint + forceScale * fNormal * normalDir;
				vSystem->graphicsData.AddLine(midPoint, forceTip, forceColor, forceColor, itemID);
				if (drawValues)
				{
					STDstring text = EXUstd::Num2String(fNormal, visualizationSettings.general.rendererPrecision);
					vSystem->graphicsData.AddText(forceTip, forceColor, text, 0.f, 0.f, 0.f, itemID);
				}
			}
		}
	...
	if (visualizationSettings.connectors.showNumbers) { 
		EXUvis::DrawItemNumber(0.5*(pos[0]+pos[1]), vSystem, itemID, "", currentColor); 
	}
}

void VisualizationObjectContactFewTeeth::UpdateGraphics(const VisualizationSettings& visualizationSettings, VisualizationSystem* vSystem, Index itemNumber)
{
#ifndef EXUDYN_MINIMAL_COMPILATION
	if (!show)
	{
	...
	Index itemID = Index2ItemID(itemNumber, ItemType::Object, vSystem->GetSystemID());
	Float4 currentColor = (this->color[0] != -1.f) ? this->color : visualizationSettings.connectors.defaultColor;

	CObjectContactFewTeeth* cItem = (CObjectContactFewTeeth*)vSystem->systemData->GetCObjects()[itemNumber];

	const ArrayIndex& markerNumbers = cItem->GetMarkerNumbers();

	Vector3D posOuter;
	Vector3D posInner;
	vSystem->systemData->GetCMarkers()[markerNumbers[0]]->GetPosition(*vSystem->systemData, posOuter, ConfigurationType::Visualization);
	vSystem->systemData->GetCMarkers()[markerNumbers[1]]->GetPosition(*vSystem->systemData, posInner, ConfigurationType::Visualization);

	Matrix3D AOuter;
	Matrix3D AInner;
	vSystem->systemData->GetCMarkers()[markerNumbers[0]]->GetRotationMatrix(*vSystem->systemData, AOuter, ConfigurationType::Visualization);
	vSystem->systemData->GetCMarkers()[markerNumbers[1]]->GetRotationMatrix(*vSystem->systemData, AInner, ConfigurationType::Visualization);

	// Draw connection line between markers (optional)
	if (visualizationSettings.connectors.showContact)
	{
		vSystem->graphicsData.AddLine(posOuter, posInner, currentColor, currentColor, itemID);
	}

	// ========== Draw contact forces and values ==========
	if (visualizationSettings.contact.showContactForces || visualizationSettings.contact.showContactForcesValues)
	{
		// Get contact force data from node
		LinkedDataVector data = ((CNodeData*)cItem->GetCNode(0))->GetVisualizationCoordinateVector();
		
		// Extract contact force and gap
		Real contactForce = data[CObjectContactFewTeeth::dataIndexContactForce];
		Real gap = data[CObjectContactFewTeeth::dataIndexGap];
		
		// Only draw if in contact (gap <= 0 and force > 0)
		if (gap <= 0.0 && contactForce > 1e-6)
		{
			// Compute marker data structure for connector
			MarkerDataStructure markerDataStructure;
			const bool computeJacobian = false;
			vSystem->systemData->ComputeMarkerDataStructure(cItem, computeJacobian, markerDataStructure);
			
			Vector valuePos;
			cItem->GetOutputVariableConnector(OutputVariableType::Position, markerDataStructure, itemNumber, valuePos);
			Vector3D contactPoint = Vector3D({ valuePos[0], valuePos[1], valuePos[2] });

			// Get contact force vector
			Vector value;
			cItem->GetOutputVariableConnector(OutputVariableType::ForceLocal, markerDataStructure, itemNumber, value);
			Vector3D forceVector({ value[0], value[1], value[2] });
			Real forceMagnitude = forceVector.GetL2Norm();
			
			if (forceMagnitude > 1e-6)
			{
				// Compute force arrow endpoint
				Vector3D forceArrow = forceVector * visualizationSettings.contact.contactForcesFactor;
				Vector3D arrowEnd = contactPoint + forceArrow;
				
				// Draw contact force arrow (red color)
				const Float4 forceColor = Float4({ 1.0f, 0.0f, 0.0f, 1.0f });
				
				if (visualizationSettings.contact.showContactForces)
				{
					vSystem->graphicsData.AddLine(contactPoint, arrowEnd, forceColor, forceColor, itemID);
					
					// Draw contact point sphere
					Real pointRadius = 0.5f * visualizationSettings.contact.contactPointsDefaultSize;
					vSystem->graphicsData.AddSphere(contactPoint, forceColor, itemID, (float)pointRadius, 12);
				}
				
				// Draw force magnitude text
				if (visualizationSettings.contact.showContactForcesValues)
				{
					vSystem->graphicsData.AddText(arrowEnd, forceColor, 
						EXUstd::Num2String(forceMagnitude, visualizationSettings.general.rendererPrecision), 
						0.f, 0.f, 0.f, itemID);
				}
			}
		}
	...
	if (visualizationSettings.connectors.showNumbers)
	{
		EXUvis::DrawItemNumber(0.5 * (posOuter + posInner), vSystem, itemID, "", currentColor);
	}
#endif
}

void VisualizationObjectContactExternalInvolute::UpdateGraphics(const VisualizationSettings& visualizationSettings, VisualizationSystem* vSystem, Index itemNumber)
{
#ifndef EXUDYN_MINIMAL_COMPILATION
	if (!show)
	{
	...
	CObjectContactExternalInvolute* cItem = (CObjectContactExternalInvolute*)vSystem->systemData->GetCObjects()[itemNumber];

	const ArrayIndex& markerNumbers = cItem->GetMarkerNumbers();

	Vector3D posGear1;
	Vector3D posGear2;
	vSystem->systemData->GetCMarkers()[markerNumbers[0]]->GetPosition(*vSystem->systemData, posGear1, ConfigurationType::Visualization);
	vSystem->systemData->GetCMarkers()[markerNumbers[1]]->GetPosition(*vSystem->systemData, posGear2, ConfigurationType::Visualization);

	// Draw connection line between markers (optional)
	if (visualizationSettings.connectors.showContact)
	{
		vSystem->graphicsData.AddLine(posGear1, posGear2, currentColor, currentColor, itemID);
	}
	...
	if (visualizationSettings.contact.showContactForces || visualizationSettings.contact.showContactForcesValues)
	{
		...
		// Extract contact force and gap
		Real contactForce = data[CObjectContactExternalInvolute::dataIndexContactForce];
		Real gap = data[CObjectContactExternalInvolute::dataIndexGap];
		...
		if (gap <= 0.0 && contactForce > 1e-6)
		{
			...
			Vector valuePos;
			cItem->GetOutputVariableConnector(OutputVariableType::Position, markerDataStructure, itemNumber, valuePos);
			// Use average Z position of the two markers (gear centers) instead of Z=0 from 2D contact computation
			Real zPosition = 0.5 * (posGear1[2] + posGear2[2]);
			Vector3D contactPoint = Vector3D({ valuePos[0], valuePos[1], zPosition });
			...
			if (forceMagnitude > 1e-6)
			{
				...
				// Draw contact force arrow (green color for external gear)
				const Float4 forceColor = Float4({ 0.0f, 1.0f, 0.0f, 1.0f });
	...
	if (visualizationSettings.connectors.showNumbers)
	{
		EXUvis::DrawItemNumber(0.5 * (posGear1 + posGear2), vSystem, itemID, "", currentColor);
	}
#endif
}
