...
void VisualizationObjectANCFCable::UpdateGraphics(const VisualizationSettings& visualizationSettings, VisualizationSystem* vSystem, Index itemNumber)
{
	...
	EXUmath::ComputeOrthogonalBasis(slopeX0, normalY0, normalZ0);
	...
	for (Index i = 1; i <= tiling; i++)
	{
		...
		//EXUmath::ComputeOrthogonalBasis(slopeX1, normalY1, normalZ1);
...
void VisualizationObjectContactSphereSphere::UpdateGraphics(const VisualizationSettings& visualizationSettings, VisualizationSystem* vSystem, Index itemNumber)
{
	...
	for (Index i = 0; i < 2; i++)
	{
		EXUvis::DrawSphere(pos[i], radius[i], currentColor, vSystem->graphicsData, itemID, visualizationSettings.general.sphereTiling);
	}
...
void VisualizationObjectContactCurveCircles::UpdateGraphics(const VisualizationSettings& visualizationSettings, VisualizationSystem* vSystem, Index itemNumber)
{
	...
	Vector3D pos0;
	vSystem->systemData->GetCMarkers()[cItem->GetMarkerNumbers()[0]]->GetPosition(*vSystem->systemData, pos0, ConfigurationType::Visualization);
	//vSystem->systemData->GetCMarkers()[cItem->GetMarkerNumbers()[1]]->GetPosition(*vSystem->systemData, pos[1], ConfigurationType::Visualization);
	...
	if (visualizationSettings.connectors.showNumbers) { EXUvis::DrawItemNumber(pos0, vSystem, itemID, "", currentColor); }
}
