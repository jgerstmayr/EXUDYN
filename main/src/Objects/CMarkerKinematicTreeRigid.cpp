...
{
	position = ((CObjectBody*)(cSystemData.GetCObjects()[parameters.objectNumber]))->GetPosition(parameters.localPosition, configuration);
}
...
{
	velocity = ((CObjectBody*)(cSystemData.GetCObjects()[parameters.objectNumber]))->GetVelocity(parameters.localPosition, configuration);
}
...
void CMarkerKinematicTreeRigid::GetRotationMatrix(const CSystemData& cSystemData, Matrix3D& rotationMatrix, ConfigurationType configuration) const
{
	...
	rotationMatrix = ((CObjectBody*)(cSystemData.GetCObjects()[parameters.objectNumber]))->GetRotationMatrix(parameters.localPosition, configuration);
}
...
{
	angularVelocity = ((CObjectBody*)(cSystemData.GetCObjects()[parameters.objectNumber]))->GetAngularVelocity(parameters.localPosition, configuration);
}
...
{
	angularVelocity = ((CObjectBody*)(cSystemData.GetCObjects()[parameters.objectNumber]))->GetAngularVelocityLocal(parameters.localPosition, configuration);
}
