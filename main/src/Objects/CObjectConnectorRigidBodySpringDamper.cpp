	...
	Matrix3D& Ajoint, Vector3D& vLocPos, Vector3D& vLocVel, Vector3D& vLocRot, Vector3D& vLocAngVel, Vector6D& fLocVec6D) const
{
	...
	{
		HomogeneousTransformation HT0(A0all, markerData.GetMarkerData(0).position);
		HomogeneousTransformation HT1(A1all, markerData.GetMarkerData(1).position);

		HT0.GetRelativeMotionTo(HT1, vLocPos, vLocRot);

		//HT1 = relative homogeneous transformation to mid point/rotation!
		HT1 = EXUlie::ExpSE3(0.5*vLocPos, 0.5*vLocRot); 
		HT0 *= HT1; //HT0 = SE3 mid-frame
		Ajoint = HT0.GetRotation();
	}
	else
	{
		Ajoint = A0all;
		vLocPos = Ajoint.GetTransposed() * (markerData.GetMarkerData(1).position - markerData.GetMarkerData(0).position); //vLocPos transformed to marker0 local coordinate system, where springs are defined
		Matrix3D relRot = Ajoint.GetTransposed() * A1all;
		vLocRot = RigidBodyMath::RotationMatrix2RotXYZ(relRot);
	}

	//relative velocities: same in all formulations
	vLocVel = Ajoint.GetTransposed() * (markerData.GetMarkerData(1).velocity - markerData.GetMarkerData(0).velocity);
	vLocAngVel = Ajoint.GetTransposed()*(A1*markerData.GetMarkerData(1).angularVelocityLocal - A0 * markerData.GetMarkerData(0).angularVelocityLocal);
...
void CObjectConnectorRigidBodySpringDamper::ComputeODE2LHS(Vector& ode2Lhs, const MarkerDataStructure& markerData, Index objectNumber) const
{
	...
	if (parameters.activeConnector)
	{
		...
		Vector6D fLocVec6D;
		Matrix3D Ajoint;
		ComputeSpringForceTorque(markerData, objectNumber, Ajoint, vLocPos, vLocVel, vLocRot, vLocAngVel, fLocVec6D);
		...
		if (markerData.GetMarkerData(1).positionJacobian.NumberOfColumns()) //special case: COGround has (0,0) Jacobian
		{
			...
			EXUmath::MultMatrixTransposedVectorAdd(markerData.GetMarkerData(1).rotationJacobian, fRot+torque, ldv1);
		}
		...
		if (markerData.GetMarkerData(0).positionJacobian.NumberOfColumns()) //special case: COGround has (0,0) Jacobian
		{
			...
			EXUmath::MultMatrixTransposedVectorAdd(markerData.GetMarkerData(0).rotationJacobian, fRot+torque, ldv0);
		}
...
Real CObjectConnectorRigidBodySpringDamper::PostNewtonStep(const MarkerDataStructure& markerDataCurrent, Index itemIndex, PostNewtonFlags::Type& flags, Real& recommendedStepSize)
{
	...
	if (parameters.postNewtonStepUserFunction && parameters.activeConnector && parameters.nodeNumber != EXUstd::InvalidIndex)
	{
		...
		//adds some overhead ...
		ComputeSpringForceTorque(markerDataCurrent, itemIndex, Ajoint, vLocPos, vLocVel, vLocRot, vLocAngVel, fLocVec6D);
