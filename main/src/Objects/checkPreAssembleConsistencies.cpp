...
#include "Autogenerated/MainObjectANCFThinPlate.h"
...
#include "Autogenerated/MainObjectJointSliding.h"
...
bool MainObjectANCFThinPlate::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
...
bool MainObjectBeamGeometricallyExact2D::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	CObjectBeamGeometricallyExact2D* cObject = (CObjectBeamGeometricallyExact2D*)GetCObject();

	//check for valid node number already done prior to this function
	if (cObject->GetNumberOfNodes() < 2 || cObject->GetNumberOfNodes() > 3)
	{
		errorString = "ObjectBeamGeometricallyExact2D: requires 2 or 3 nodes (linear or quadratic element)";
		return false;
	}

	for (Index i = 0; i < 2; i++) {
		if (std::strcmp(mainSystem.GetMainSystemData().GetMainNode(cObject->GetNodeNumber(i)).GetTypeName(), "RigidBody2D") != 0)
		{
			errorString = "ObjectBeamGeometricallyExact2D: node " + EXUstd::ToString(i) + " must be of type 'RigidBody2D', but found type '" +
				mainSystem.GetMainSystemData().GetMainNode(cObject->GetNodeNumber(i)).GetTypeName() + "'";
			return false;
		}
	}

	LinkedDataVector p0 = ((CNodeODE2*)cObject->GetCNode(0))->GetReferenceCoordinateVector();
	LinkedDataVector p1 = ((CNodeODE2*)cObject->GetCNode(1))->GetReferenceCoordinateVector();

	if ((p1 - p0).GetL2Norm() == 0)
	{
		errorString = "ObjectBeamGeometricallyExact2D: nodes must have different reference positions, but found that node 0 and node 1 have identical positions";
		return false;
	}

	if (!cObject->IsLinear())
	{
		LinkedDataVector p2 = ((CNodeODE2*)cObject->GetCNode(2))->GetReferenceCoordinateVector();
		if ((p2 - p0).GetL2Norm() == 0 || (p2 - p1).GetL2Norm() == 0)
		{
			errorString = "ObjectBeamGeometricallyExact2D: nodes must have different reference positions, but found that node 2 and node 0 (or 1) have identical positions";
			return false;
		}
	}
...
bool MainObjectContactSphereTriangle::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	...
	if (!EXUstd::IsOfTypeAndNotNone((Index)mainSystem.GetCSystem().GetSystemData().GetCMarker(nMarkers[1]).GetType(), 
		(Index)(Marker::Position + Marker::Orientation)) )
	{
		errorString = STDstring("CObjectContactSphereTriangle: Marker 1 must generally be of type = 'Rigid' (Position + Orientation)");
		return false;
	}
...
bool MainObjectJointSliding::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	CObjectJointSliding* cObject = (CObjectJointSliding*)GetCObject();
	...
	{
		errorString = "ObjectJointSliding: the slidingMarkerNumbers list (size=" + EXUstd::ToString(slidingMarkerSize) + ") must have same size as the slidingMarkerOffsets list (size=" + EXUstd::ToString(slidingMarkerOffsetSize) + ")";
		return false;
	}

	if (initialMarker < 0 || (Index)initialMarker >= slidingMarkerSize)
	{
		errorString = "ObjectJointSliding: initial Data variable must be >= 0 and < " + EXUstd::ToString(slidingMarkerSize);
		return false;
	}
	...
	{
		errorString = "ObjectJointSliding: node must be of type 'GenericData'";
		return false;
	}
	...
	{
		errorString = STDstring("ObjectJointSliding: NodeGenericData (Node ") + EXUstd::ToString(dataNodeNumber) + ") must have " +
	...
	const ArrayIndex& nMarkers = cObject->GetMarkerNumbers();
	if (!(cObject->GetParameters().constrainRotations == 0) )
	{
		...
		{
			errorString = STDstring("ObjectJointSliding: in case of constrainRotations[i] != 0, Marker 0 must be of type = 'Position' + 'Orientation'");
			return false;
		}
	}
	else if (!EXUstd::IsOfType(mainSystem.GetCSystem().GetSystemData().GetCMarker(nMarkers[0]).GetType(), Marker::Position))
	{
		errorString = STDstring("ObjectJointSliding: Marker 0 must be of type = 'Position'");
		return false;
	}

	if (STDstring(mainSystem.GetMainSystemData().GetMainMarkers()[nMarkers[1]]->GetTypeName()) != STDstring("BodyBeamShape"))
	{
		errorString = STDstring("ObjectJointSliding: Marker 1 must be of type = 'BodyBeamShape'");
		return false;
	}
