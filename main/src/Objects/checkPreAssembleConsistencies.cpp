...
#include "Autogenerated/MainObjectCustomContact.h"
#include "Autogenerated/MainObjectContactCircleCircle.h"
...
bool MainObjectCustomContact::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	CObjectCustomContact* cObject = (CObjectCustomContact*)GetCObject();
	Index node = cObject->GetNodeNumber(0);

	if (cObject->GetParameters().nodeNumber == EXUstd::InvalidIndex ||
		cObject->GetParameters().nodeNumber >= mainSystem.GetMainSystemData().GetMainNodes().NumberOfItems())
	{
		errorString = "CObjectCustomContact: node number is invalid!";
		return false;
	}
	...
	if (std::strcmp(mainSystem.GetMainSystemData().GetMainNode(node).GetTypeName(), "GenericData") != 0)
	{
		errorString = "CObjectCustomContact: node must be of type 'GenericData'";
		return false;
	}
	...
	if (nc != cObject->GetDataVariablesSize())
	{
		errorString = STDstring("CObjectCustomContact: NodeGenericData (Node ") + 
			EXUstd::ToString(node) + ") must have " + EXUstd::ToString(cObject->GetDataVariablesSize())
			+ " coordinates (found: " + EXUstd::ToString(nc) + ")";
		return false;
	}
	...
	const ArrayIndex& nMarkers = cObject->GetMarkerNumbers();
	CHECKandTHROW(nMarkers.NumberOfItems() >= 2, "CObjectCustomContact: number of markers must be 2");

	Index requestedMarkerType = (Index)(Marker::Position);
	if (cObject->GetParameters().frictionCoefficient != 0.) { requestedMarkerType += (Index)Marker::Orientation; }

	if (!EXUstd::IsOfTypeAndNotNone((Index)mainSystem.GetCSystem().GetSystemData().GetCMarker(nMarkers[0]).GetType(), requestedMarkerType))
	{
		errorString = STDstring("CObjectCustomContact: Marker 0 must be of type 'Position' or 'Rigid' (Position + Orientation) if friction is non-zero");
		return false;
	}

	if (!EXUstd::IsOfTypeAndNotNone((Index)mainSystem.GetCSystem().GetSystemData().GetCMarker(nMarkers[1]).GetType(), requestedMarkerType))
	{
		errorString = STDstring("CObjectCustomContact: Marker 1 must be of type 'Position' or 'Rigid' (Position + Orientation) if friction is non-zero");
		return false;
	}

	if (cObject->GetParameters().contactStiffness <= 0.)
	{
		errorString = STDstring("CObjectCustomContact: contactStiffness must be greater than zero");
		return false;
	}

	if (cObject->GetParameters().contactDamping < 0.)
	{
		errorString = STDstring("CObjectCustomContact: contactDamping must be non-negative");
		return false;
	}

	if (cObject->GetParameters().contactReferenceDistance < 0.)
	{
		errorString = STDstring("CObjectCustomContact: contactReferenceDistance must be non-negative");
		return false;
	}

	if (cObject->GetParameters().frictionCoefficient < 0.)
	{
		errorString = STDstring("CObjectCustomContact: frictionCoefficient must be non-negative");
		return false;
	}
...
bool MainObjectContactCircleCircle::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	CObjectContactCircleCircle* cObject = (CObjectContactCircleCircle*)GetCObject();
	Index node = cObject->GetNodeNumber(0);

	if (cObject->GetParameters().nodeNumber == EXUstd::InvalidIndex ||
		cObject->GetParameters().nodeNumber >= mainSystem.GetMainSystemData().GetMainNodes().NumberOfItems())
	{
		errorString = "CObjectContactCircleCircle: node number is invalid!";
		return false;
	}
	...
	if (std::strcmp(mainSystem.GetMainSystemData().GetMainNode(node).GetTypeName(), "GenericData") != 0)
	{
		errorString = "CObjectContactCircleCircle: node must be of type 'GenericData'";
		return false;
	}
	...
	if (nc != cObject->GetDataVariablesSize())
	{
		errorString = STDstring("CObjectContactCircleCircle: NodeGenericData (Node ") + 
			EXUstd::ToString(node) + ") must have " + EXUstd::ToString(cObject->GetDataVariablesSize())
			+ " coordinates (found: " + EXUstd::ToString(nc) + ")";
		return false;
	}
	...
	const ArrayIndex& nMarkers = cObject->GetMarkerNumbers();
	CHECKandTHROW(nMarkers.NumberOfItems() >= 2, "CObjectContactCircleCircle: number of markers must be 2");

	if (!EXUstd::IsOfTypeAndNotNone((Index)mainSystem.GetCSystem().GetSystemData().GetCMarker(nMarkers[0]).GetType(), requestedMarkerType))
	{
		errorString = STDstring("CObjectContactCircleCircle: Marker 0 must be of type = 'Position'");
		return false;
	}

	if (!EXUstd::IsOfTypeAndNotNone((Index)mainSystem.GetCSystem().GetSystemData().GetCMarker(nMarkers[1]).GetType(), requestedMarkerType))
	{
		errorString = STDstring("CObjectContactCircleCircle: Marker 1 must be of type = 'Position'");
		return false;
	}

	if (cObject->GetParameters().contactStiffness < 0.)
	{
		errorString = STDstring("CObjectContactCircleCircle: contactStiffness must be non-negative");
		return false;
	}

	if (cObject->GetParameters().contactDamping < 0.)
	{
		errorString = STDstring("CObjectContactCircleCircle: contactDamping must be non-negative");
		return false;
	}

	if (cObject->GetParameters().frictionCoefficient < 0.)
	{
		errorString = STDstring("CObjectContactCircleCircle: frictionCoefficient must be non-negative");
		return false;
	}
