...
bool MainObjectConnectorLinearSpringDamper::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
    ...
    if (fabs(cObject->GetParameters().axisMarker0.GetL2Norm() - 1) > 1e-14)
    {
...
bool MainObjectContactSphereSphere::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	...
	if (!IsInRange(errorString, cObject->GetParameters().contactPlasticityRatio, 0., 1. - 1e-16,
...
bool MainObjectContactCurveCircles::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	...
	////for circles, we only need position markers if there is no friction:
	//Index requestedMarkerType = (Index)(Marker::Position);
	//if (cObject->GetParameters().dynamicFriction != 0.) { requestedMarkerType += (Index)Marker::Orientation; }

	//if (!EXUstd::IsOfTypeAndNotNone((Index)mainSystem.GetCSystem().GetSystemData().GetCMarker(nMarkers[1]).GetType(), requestedMarkerType))
	//{
	//	errorString = STDstring("CObjectContactCurveCircles: Marker 1 must generally be of type = 'Rigid' (Position + Orientation) or of type Position, if friction is zero");
	//	return false;
	//}

	pout << "CObjectContactCurveCircles::CheckPreAssembleConsistency: add further checks for marker types of circles + consistency of radii lists and data + dense matrix in MC + friction=0 for now + check that all circle markers have same dimension!\n";
