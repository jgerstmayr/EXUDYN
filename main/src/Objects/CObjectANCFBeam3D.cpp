/** ***********************************************************************************************
* @brief        CObjectANCFBeam3D implementation
*
...
#define includeCObjectANCFBeam3D

#ifdef includeCObjectANCFBeam3D
...
#include "Autogenerated/MainObjectANCFBeam3D.h"
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//without Main object:
...
void MainObjectANCFBeam3D::SetInternalBeamSection(const py::object& pyObject)
{
	if (py::isinstance<PyBeamSection>(pyObject)) //this must be the C-object
	{
		PyBeamSection bs(py::cast<PyBeamSection>(pyObject));
		GetCObjectANCFBeam3D()->GetParameters().physicsAxialShearStiffness =
			Vector3D({ bs.stiffnessMatrix(0,0),bs.stiffnessMatrix(1,1),bs.stiffnessMatrix(2,2) });
		GetCObjectANCFBeam3D()->GetParameters().physicsTorsionalBendingStiffness =
			Vector3D({ bs.stiffnessMatrix(3,3),bs.stiffnessMatrix(4,4),bs.stiffnessMatrix(5,5) });

		GetCObjectANCFBeam3D()->GetParameters().physicsCrossSectionInertia = bs.inertia;
		GetCObjectANCFBeam3D()->GetParameters().physicsMassPerLength = bs.massPerLength;
		...
		{
			PyError("ObjectANCFBeam3D: BeamSection stiffnessMatrix contains values which can not be used");
		}
		if (!(bs.inertia == bsCheck.inertia))
		{
			PyError("ObjectANCFBeam3D: BeamSection inertia contains values which can not be used");
		}
	}
	else
	{
		PyError("ObjectANCFBeam3D: expected BeamSection, but received: " + STDstring(py::str(pyObject)));
	}
}

//! AUTO: special function which returns BeamSection converted from local data
PyBeamSection MainObjectANCFBeam3D::GetInternalBeamSection() const
{
	PyBeamSection bs;
	Vector3D kAS = GetCObjectANCFBeam3D()->GetParameters().physicsAxialShearStiffness;
	bs.stiffnessMatrix(0, 0) = kAS[0];
	bs.stiffnessMatrix(1, 1) = kAS[1];
	bs.stiffnessMatrix(2, 2) = kAS[2];

	Vector3D kKappa = GetCObjectANCFBeam3D()->GetParameters().physicsTorsionalBendingStiffness;
	bs.stiffnessMatrix(3, 3) = kKappa[0];
	bs.stiffnessMatrix(4, 4) = kKappa[1];
	bs.stiffnessMatrix(5, 5) = kKappa[2];

	bs.inertia = GetCObjectANCFBeam3D()->GetParameters().physicsCrossSectionInertia;
	bs.massPerLength = GetCObjectANCFBeam3D()->GetParameters().physicsMassPerLength;
...
SlimVector<CObjectANCFBeam3D::nSFperNode*CObjectANCFBeam3D::nNodes> CObjectANCFBeam3D::ComputeShapeFunctions(const Vector3D& localPosition, Real L) 
{
	...
	return SlimVector<CObjectANCFBeam3D::nSFperNode * CObjectANCFBeam3D::nNodes>(
		{ val1, val1 * y, val1 * z, val4, val4 * y, val4 * z });
}


//! get derivative of compressed shape function vector for slopes
SlimVector<CObjectANCFBeam3D::nSFperNode*CObjectANCFBeam3D::nNodes> CObjectANCFBeam3D::ComputeShapeFunctions_x(const Vector3D& localPosition, Real L) 
{
	Real fact = 1. / L;
...
//! get derivative of compressed shape function vector for slopes
SlimVector<CObjectANCFBeam3D::nSFperNode*CObjectANCFBeam3D::nNodes> CObjectANCFBeam3D::ComputeShapeFunctions_y(const Vector3D& localPosition, Real L) 
{
...
//! get derivative of compressed shape function vector for slopes
SlimVector<CObjectANCFBeam3D::nSFperNode*CObjectANCFBeam3D::nNodes> CObjectANCFBeam3D::ComputeShapeFunctions_z(const Vector3D& localPosition, Real L) 
{
	Real x0 = localPosition[0] / L;
	Real val1 = 0.5 - x0;
	Real val4 = 0.5 + x0;
...
//! get derivative of compressed shape function vector for slopes
SlimVector<CObjectANCFBeam3D::nSFperNode*CObjectANCFBeam3D::nNodes> CObjectANCFBeam3D::ComputeShapeFunctions_yx(const Vector3D& localPosition, Real L) 
{
	Real fact = 1. / L;
	return SlimVector<CObjectANCFBeam3D::nSFperNode * CObjectANCFBeam3D::nNodes>(
		{ 0., -fact, 0., 0., fact, 0. });
...
//! get derivative of compressed shape function vector for slopes
SlimVector<CObjectANCFBeam3D::nSFperNode*CObjectANCFBeam3D::nNodes> CObjectANCFBeam3D::ComputeShapeFunctions_zx(const Vector3D& localPosition, Real L) 
{
	Real fact = 1. / L;
	return SlimVector<CObjectANCFBeam3D::nSFperNode * CObjectANCFBeam3D::nNodes>(
		{ 0., 0., -fact, 0., 0., fact });
...
void CObjectANCFBeam3D::ComputeCurrentNodeCoordinates(ConstSizeVector<CObjectANCFBeam3D::nODE2perNode>& qNode0, ConstSizeVector<CObjectANCFBeam3D::nODE2perNode>& qNode1) const
{
...
void CObjectANCFBeam3D::ComputeCurrentNodeVelocities(ConstSizeVector<CObjectANCFBeam3D::nODE2perNode>& qNode0, ConstSizeVector<CObjectANCFBeam3D::nODE2perNode>& qNode1) const
{
...
void CObjectANCFBeam3D::ComputeCurrentObjectCoordinates(ConstSizeVector<CObjectANCFBeam3D::nODE2coordinates>& qANCF) const
{
...
void CObjectANCFBeam3D::ComputeCurrentObjectVelocities(ConstSizeVector<CObjectANCFBeam3D::nODE2coordinates>& qANCF_t) const
{
...
void CObjectANCFBeam3D::ComputeSlopeVectors(Real x, ConfigurationType configuration, Vector3D& slopeX, Vector3D& slopeY, Vector3D& slopeZ) const
{
...
void CObjectANCFBeam3D::PreComputeMassTerms() const
{
...
//templates need to be defined in .h file ==> put it outside of CObjectANCFBeam3D
...
template <class TReal>
void GetLocalTwistAndCurvatureDiff(const ConstSizeVectorBase<TReal, CObjectANCFBeam3D::nODE2coordinates>& qANCF,
	ConstSizeVectorBase<TReal, EXUstd::dim3D>& f, Real L, Real x)
{
	const Index nSFperNode = CObjectANCFBeam3D::nSFperNode;
	Vector3D localPosition({ x,0.,0. });
	LinkedDataVectorBase<TReal> q0(qANCF, 0, CObjectANCFBeam3D::nODE2perNode);		//link node values to element vector
	LinkedDataVectorBase<TReal> q1(qANCF, CObjectANCFBeam3D::nODE2perNode, CObjectANCFBeam3D::nODE2perNode);		//link node values to element vector

	//SlimVectorBase<TReal, 3> slopeX = ExuMath::MapCoordinates2Nodes<Real, LinkedDataVectorBase<TReal>, nSFperNode, EXUstd::dim3D>(CObjectANCFBeam3D::ComputeShapeFunctions_x(localPosition, L), q0, q1);
	SlimVectorBase<TReal, 3> slopeY  = ExuMath::MapCoordinates2Nodes<TReal, LinkedDataVectorBase<TReal>, nSFperNode, EXUstd::dim3D>(CObjectANCFBeam3D::ComputeShapeFunctions_y(localPosition, L), q0, q1);
	SlimVectorBase<TReal, 3> slopeZ  = ExuMath::MapCoordinates2Nodes<TReal, LinkedDataVectorBase<TReal>, nSFperNode, EXUstd::dim3D>(CObjectANCFBeam3D::ComputeShapeFunctions_z(localPosition, L), q0, q1);
	SlimVectorBase<TReal, 3> slopeYX = ExuMath::MapCoordinates2Nodes<TReal, LinkedDataVectorBase<TReal>, nSFperNode, EXUstd::dim3D>(CObjectANCFBeam3D::ComputeShapeFunctions_yx(localPosition, L), q0, q1);
	SlimVectorBase<TReal, 3> slopeZX = ExuMath::MapCoordinates2Nodes<TReal, LinkedDataVectorBase<TReal>, nSFperNode, EXUstd::dim3D>(CObjectANCFBeam3D::ComputeShapeFunctions_zx(localPosition, L), q0, q1);
...
void CObjectANCFBeam3D::GetDeltaLocalTwistAndCurvature(Real x, ConstSizeMatrix<EXUstd::dim3D * CObjectANCFBeam3D::nODE2coordinates>& deltaKappa, 
...
//typedef Real TReal;
template <class TReal>
void GetLocalAxialShearDeformationDiff(const ConstSizeVectorBase<TReal, CObjectANCFBeam3D::nODE2coordinates>& qANCF,
	ConstSizeVectorBase<TReal, EXUstd::dim3D>& f, Real L, Real x)
{
	Vector3D localPosition({ x,0.,0. });
	LinkedDataVectorBase<TReal> q0(qANCF, 0, CObjectANCFBeam3D::nODE2perNode);		//link node values to element vector
	...
	SlimVectorBase<TReal, 3> slopeX = ExuMath::MapCoordinates2Nodes<TReal, LinkedDataVectorBase<TReal>, CObjectANCFBeam3D::nSFperNode,
		EXUstd::dim3D>(CObjectANCFBeam3D::ComputeShapeFunctions_x(localPosition, L), q0, q1);
	SlimVectorBase<TReal, 3> slopeY = ExuMath::MapCoordinates2Nodes<TReal, LinkedDataVectorBase<TReal>, CObjectANCFBeam3D::nSFperNode,
		EXUstd::dim3D>(CObjectANCFBeam3D::ComputeShapeFunctions_y(localPosition, L), q0, q1);
	SlimVectorBase<TReal, 3> slopeZ = ExuMath::MapCoordinates2Nodes<TReal, LinkedDataVectorBase<TReal>, CObjectANCFBeam3D::nSFperNode,
		EXUstd::dim3D>(CObjectANCFBeam3D::ComputeShapeFunctions_z(localPosition, L), q0, q1);
...
void CObjectANCFBeam3D::GetDeltaLocalAxialShearDeformation(Real x, ConstSizeMatrix<EXUstd::dim3D * CObjectANCFBeam3D::nODE2coordinates>& deltaAxialShearDeformation,
...
////typedef Real TReal;
//template <class TReal>
//void GetCrossSectionDeformationDiff(const ConstSizeVectorBase<TReal, CObjectANCFBeam3D::nODE2coordinates>& qANCF,
//	ConstSizeVectorBase<TReal, EXUstd::dim3D>& f, Real L, Real x)
//{
//	Vector3D localPosition({ x,0.,0. });
//	LinkedDataVector q0(qANCF, 0, CObjectANCFBeam3D::nODE2perNode);		//link node values to element vector
//	LinkedDataVector q1(qANCF, CObjectANCFBeam3D::nODE2perNode, CObjectANCFBeam3D::nODE2perNode);		//link node values to element vector
//
//	SlimVectorBase<TReal, 3> slopeX = ExuMath::MapCoordinates2Nodes<Real, LinkedDataVector, CObjectANCFBeam3D::nSFperNode,
//		EXUstd::dim3D>(CObjectANCFBeam3D::ComputeShapeFunctions_x(localPosition, L), q0, q1);
//	SlimVectorBase<TReal, 3> slopeY = ExuMath::MapCoordinates2Nodes<Real, LinkedDataVector, CObjectANCFBeam3D::nSFperNode,
//		EXUstd::dim3D>(CObjectANCFBeam3D::ComputeShapeFunctions_y(localPosition, L), q0, q1);
//	SlimVectorBase<TReal, 3> slopeZ = ExuMath::MapCoordinates2Nodes<Real, LinkedDataVector, CObjectANCFBeam3D::nSFperNode,
//		EXUstd::dim3D>(CObjectANCFBeam3D::ComputeShapeFunctions_z(localPosition, L), q0, q1);
//	//SlimVectorBase<TReal, 3> slopeYX = ExuMath::MapCoordinates2Nodes<Real, LinkedDataVector, nSFperNode, EXUstd::dim3D>(ComputeShapeFunctions_yx(localPosition, L), q0, q1);
//	//SlimVectorBase<TReal, 3> slopeZX = ExuMath::MapCoordinates2Nodes<Real, LinkedDataVector, nSFperNode, EXUstd::dim3D>(ComputeShapeFunctions_zx(localPosition, L), q0, q1);
//
...
void CObjectANCFBeam3D::GetDeltaCrossSectionDeformation(Real x, ConstSizeMatrix<EXUstd::dim3D * CObjectANCFBeam3D::nODE2coordinates>& deltaCrossSectionDeformation,
...
//! Computational function: compute mass matrix
void CObjectANCFBeam3D::ComputeMassMatrix(EXUmath::MatrixContainer& massMatrixC, const ArrayIndex& ltg, Index objectNumber) const
{
...
void CObjectANCFBeam3D::ComputeODE2LHS(Vector& ode2Lhs, Index objectNumber) const
{
	ConstSizeVector<nODE2coordinates> qANCF;
...
//! Computational function: compute left-hand-side (LHS) of second order ordinary differential equations (ODE) to "ode2Lhs"
template<class TReal>
void CObjectANCFBeam3D::ComputeODE2LHStemplate(VectorBase<TReal>& ode2Lhs, 
...
////void CObjectANCFBeam3D::ComputeJacobianODE2_ODE2(ResizableMatrix& jacobian, ResizableMatrix& jacobian_ODE2_t) const
//void CObjectANCFBeam3D::ComputeJacobianODE2_ODE2(EXUmath::MatrixContainer& jacobianODE2, JacobianTemp& temp, 
...
AccessFunctionType CObjectANCFBeam3D::GetAccessFunctionTypes() const
{
...
//OutputVariableType CObjectANCFBeam3D::GetOutputVariableTypes() const
//{
//	return (OutputVariableType)((Index)OutputVariableType::Position + (Index)OutputVariableType::Velocity);
//}

//! provide Jacobian at localPosition in "value" according to object access
void CObjectANCFBeam3D::GetAccessFunctionBody(AccessFunctionType accessType, const Vector3D& localPosition, Matrix& value) const
{
	Real L = parameters.physicsLength;
	...
	{
		SlimVector<CObjectANCFBeam3D::nSFperNode*CObjectANCFBeam3D::nNodes> SV = ComputeShapeFunctions(localPosition, L);
	...
	{
		CHECKandTHROWstring("CObjectANCFBeam3D::GetAccessFunctionBody(AngularVelocity_qt): not implemented!");
	...
	case AccessFunctionType::DisplacementMassIntegral_q:
	{
		...
		//	"CObjectANCFBeam3D::GetAccessFunctionBody(DisplacementMassIntegral_q): only implemented for y=z=0!");
		...
		SlimVector<CObjectANCFBeam3D::nSFperNode*CObjectANCFBeam3D::nNodes> SV(0.);

		for (auto item : EXUmath::gaussRuleOrder3Points)
		{
			Real x = 0.5*(b - a)*item + 0.5*(b + a);
			SlimVector<CObjectANCFBeam3D::nSFperNode*CObjectANCFBeam3D::nNodes> SVloc = ComputeShapeFunctions(Vector3D({x,0.,0.}), L);
	...
	default:
		SysError("CObjectANCFBeam3D:GetAccessFunctionBody illegal accessType");
	}
}

//! provide according output variable in "value"
void CObjectANCFBeam3D::GetOutputVariableBody(OutputVariableType variableType, const Vector3D& localPosition, ConfigurationType configuration, Vector& value, Index objectNumber) const
{
	//outputVariables = "{
	...
	//	//CHECKandTHROW(y == 0., "CObjectANCFBeam3D::GetOutputVariableBody: Y-component of localPosition must be zero for Director1");
	...
	//{
	//	//CHECKandTHROW(y == 0., "CObjectANCFBeam3D::GetOutputVariableBody: Y-component of localPosition must be zero for StrainLocal");
	...
	//{
	//	//CHECKandTHROW(y == 0., "CObjectANCFBeam3D::GetOutputVariableBody: Y-component of localPosition must be zero for CurvatureLocal");
	//	value.SetVector({ ComputeCurvature(x, configuration) });
	//	break;
	//}
	//case OutputVariableType::ForceLocal: {
	//	//do not add this due to drawing function: CHECKandTHROW(y == 0., "CObjectANCFBeam3D::GetOutputVariableBody: Y-component of localPosition must be zero for ForceLocal");
	...
	//	//do not add this due to drawing function: CHECKandTHROW(y == 0., "CObjectANCFBeam3D::GetOutputVariableBody: Y-component of localPosition must be zero for TorqueLocal");
	...
	//}
	default:
		SysError("CObjectANCFBeam3D::GetOutputVariableBody failed"); //error should not occur, because types are checked!
	}
}

//  return the (global) position of "localPosition" according to configuration type
Vector3D CObjectANCFBeam3D::GetPosition(const Vector3D& localPosition, ConfigurationType configuration) const
{
...
Vector3D CObjectANCFBeam3D::GetVelocity(const Vector3D& localPosition, ConfigurationType configuration) const
{
	SlimVector<nSFperNode*nNodes> SV = ComputeShapeFunctions(localPosition, parameters.physicsLength);
...
//  return the (global) velocity of "localPosition" according to configuration type
Vector3D CObjectANCFBeam3D::GetAcceleration(const Vector3D& localPosition, ConfigurationType configuration) const
{
	SlimVector<nSFperNode*nNodes> SV = ComputeShapeFunctions(localPosition, parameters.physicsLength);
...
//! return the (global) position of "localPosition" according to configuration type
Vector3D CObjectANCFBeam3D::GetDisplacement(const Vector3D& localPosition, ConfigurationType configuration) const
{
	SlimVector<nSFperNode*nNodes> SV = ComputeShapeFunctions(localPosition, parameters.physicsLength);
...
Matrix3D CObjectANCFBeam3D::GetRotationMatrix(const Vector3D& localPosition, ConfigurationType configuration) const
{
...
HomogeneousTransformation CObjectANCFBeam3D::GetLocalPositionFrame(const Vector3D& localPosition,
...
Vector3D CObjectANCFBeam3D::GetAngularVelocity(const Vector3D& localPosition, ConfigurationType configuration) const
{
	CHECKandTHROWstring("ObjectANCFBeam3DBase::GetAngularVelocity: not implemented!");
	return Vector3D();
...
//Vector3D CObjectANCFBeam3D::GetAngularAcceleration(const Vector3D& localPosition, ConfigurationType configuration) const
//{
//	CHECKandTHROWstring("ObjectANCFBeam3DBase::GetAngularVelocity: not implemented!");
//	return Vector3D();
//}
...
//Vector2D CObjectANCFBeam3D::ComputeSlopeVector_x(Real x, ConfigurationType configuration) const
//{
...
//Real CObjectANCFBeam3D::ComputeAxialStrain(Real x, ConfigurationType configuration) const 
//{
...
//Real CObjectANCFBeam3D::ComputeCurvature(Real x, ConfigurationType configuration) const
//{
//	Vector2D rx = ComputeSlopeVector(x, configuration);
