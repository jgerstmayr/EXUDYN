...
py::object MainObjectANCFCable2D::CallFunction(STDstring functionName, py::dict args) const
{
	...
	else if (functionName == "GetAngularVelocity")
	{
		Vector3D locPos;
		STDstring configStr;
		ConfigurationType configType = ConfigurationType::None;
		...
		Vector3D v(GetCObjectBody()->GetAngularVelocity(locPos, configType));
		return py::array_t<Real>(v.NumberOfItems(), v.GetDataPointer());
	}
	else if (functionName == "GetNodeNumber")
	{
		...
		{
			PyError(STDstring("ObjectANCFCable2D::CallFunction: invalid node number ") + EXUstd::ToString(localNode) + "; must be in range [0,1]");
		}
	}
