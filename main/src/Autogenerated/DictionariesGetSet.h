...
// AUTO:  ++++++++++++++++++++++
 ...
 //add namespace for access to dictionaries  //! AUTO: read access to structure; converting into dictionary
  ...
  py::dict GetDictionaryWithTypeInfo(const VSettingsOpenGL& data) {
    ...
    d["type"] = "Vector";
    d["size"] = std::vector<int>{4};
    d["description"] = "4f position vector of GL\\_LIGHT0; 4th value should be 0 for lights like sun, but 1 for directional lights (and for attenuation factor being calculated); light0 is also used for shadows, so you need to adjust this position; see opengl manuals";
    ...
    d["type"] = "bool";
    d["size"] = std::vector<int>{1};
    d["description"] = "enlighten also backside of object; may cause problems on some graphics cards and lead to slower performance; maps to OpenGL glLightModeli(GL\\_LIGHT\\_MODEL\\_TWO\\_SIDE,...)";
  ...
  void SetDictionary(VSettingsOpenVR& data, const py::dict& d) {
    data.enableOpenVR = py::cast<bool>(d["enableOpenVR"]);
