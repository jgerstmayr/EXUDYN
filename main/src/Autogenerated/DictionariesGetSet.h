...
// AUTO:  ++++++++++++++++++++++
 ...
 //add namespace for access to dictionaries  //! AUTO: read access to structure; converting into dictionary
  py::dict GetDictionaryWithTypeInfo(const BeamSection& data) {
    ...
    d["value"] = data.PyGetLength();
    d["type"] = "PReal";
    d["size"] = std::vector<int>{1};
    d["description"] = "l_b [SI:m] length of beam element";
    structureDict["length"] = d;

    d = py::dict(); //reset local dict
    d["itemIdentifier"] = std::string(""); //identifier for item
    d["value"] = data.PyGetMassPerLength();
    d["type"] = "UReal";
    d["size"] = std::vector<int>{1};
    d["description"] = "\\rho A\\, [SI:kg/m] mass per unit length of the beam";
    structureDict["massPerLength"] = d;

    d = py::dict(); //reset local dict
    d["itemIdentifier"] = std::string(""); //identifier for item
    d["value"] = data.sectionalDampingMatrix;
    d["type"] = "Matrix6D";
    d["size"] = std::vector<int>{1};
    d["description"] = "\\LU{c}{\\Dm} \\in \\Rcal^{6 \\times 6}\\, [SI:Nsm^2, Nsm and Ns (mixed)] sectional linear damping matrix related to \\vp{\\LU{c}{\\nv}}{\\LU{c}{\\mv}} = \\LU{c}{\\Dm} \\vp{\\LU{c}{\\tepsDot}}{\\LU{c}{\\tkappaDot}}; note that this damping models is highly simplified and usually, it cannot be derived from material parameters; however, it can be used to adjust model damping to observed damping behavior.";
    structureDict["sectionalDampingMatrix"] = d;

    d = py::dict(); //reset local dict
    d["itemIdentifier"] = std::string(""); //identifier for item
    d["value"] = data.sectionalStiffnessMatrix;
    d["type"] = "Matrix6D";
    d["size"] = std::vector<int>{1};
    d["description"] = "\\LU{c}{\\Cm} \\in \\Rcal^{6 \\times 6}\\, [SI:Nm^2, Nm and N (mixed)] sectional stiffness matrix related to \\vp{\\LU{c}{\\nv}}{\\LU{c}{\\mv}} = \\LU{c}{\\Cm} \\vp{\\LU{c}{\\teps}}{\\LU{c}{\\tkappa}} with sectional normal force \\LU{c}{\\nv}, torque \\LU{c}{\\mv}, strain \\LU{c}{\\teps} and curvature \\LU{c}{\\tkappa}, all quantities expressed in the cross section frame c.";
    structureDict["sectionalStiffnessMatrix"] = d;

    return structureDict;
  }
  //! AUTO: write access to data structure; converting dictionary d into structure
  void SetDictionary(BeamSection& data, const py::dict& d) {
    data.length = py::cast<Real>(d["length"]);
    data.massPerLength = py::cast<Real>(d["massPerLength"]);
    data.sectionalDampingMatrix = py::cast<Matrix6D>(d["sectionalDampingMatrix"]);
    data.sectionalStiffnessMatrix = py::cast<Matrix6D>(d["sectionalStiffnessMatrix"]);
  }
  ...
  py::dict GetDictionaryWithTypeInfo(const BeamSectionGeometry& data) {
    ...
    d = py::dict(); //reset local dict
    d["itemIdentifier"] = std::string(""); //identifier for item
    d["value"] = data.PyGetCrossSectionRadiusY();
    d["type"] = "UReal";
    d["size"] = std::vector<int>{1};
    d["description"] = "c_Y\\, [SI:m] Y radius for circular cross section";
    structureDict["crossSectionRadiusY"] = d;

    d = py::dict(); //reset local dict
    d["itemIdentifier"] = std::string(""); //identifier for item
    d["value"] = data.PyGetCrossSectionRadiusZ();
    d["type"] = "UReal";
    d["size"] = std::vector<int>{1};
    d["description"] = "c_Z\\, [SI:m] Z radius for circular cross section";
    structureDict["crossSectionRadiusZ"] = d;

    d = py::dict(); //reset local dict
    d["itemIdentifier"] = std::string(""); //identifier for item
    d["value"] = data.crossSectionType;
    d["type"] = "CrossSectionType";
    d["size"] = std::vector<int>{1};
    d["description"] = "Type of cross section: Polygon, Circular, etc.";
    structureDict["crossSectionType"] = d;

    d = py::dict(); //reset local dict
    d["itemIdentifier"] = std::string(""); //identifier for item
    d["value"] = data.PyGetPolygonalPoints();
    d["type"] = "std::vector<Vector2D>";
    d["size"] = std::vector<int>{1};
    d["description"] = "\\pv_{pg}\\, [SI: (m,m) ] list of polygonal (Y,Z) points in local beam cross section coordinates, defined in positive rotation direction";
    structureDict["polygonalPoints"] = d;

    return structureDict;
  }
  //! AUTO: write access to data structure; converting dictionary d into structure
  void SetDictionary(BeamSectionGeometry& data, const py::dict& d) {
    data.crossSectionRadiusY = py::cast<Real>(d["crossSectionRadiusY"]);
    data.crossSectionRadiusZ = py::cast<Real>(d["crossSectionRadiusZ"]);
    data.crossSectionType = py::cast<CrossSectionType>(d["crossSectionType"]);
    data.polygonalPoints = py::cast<std::vector<Vector2D>>(d["polygonalPoints"]);
  }
  //! AUTO: read access to structure; converting into dictionary
