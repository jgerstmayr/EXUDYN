...
//        pybinding to enum classes:
  ...
  py::enum_<Node::Type>(m, "NodeType")
		...
		.value("LieGroupWithDataCoordinates", Node::LieGroupWithDataCoordinates)    //node to be solved with Lie group methods, having data coordinates
  ...
  py::enum_<Contact::TypeIndex>(m, "ContactTypeIndex")
        ...
        m.def("GetVersionString", &PyGetVersionString, "Get Exudyn built version as string (if addDetails=True, adds more information on compilation Python version, platform, etc.; the Python micro version may differ from that you are working with; AVX2 shows that you are running a AVX2 compiled version)", py::arg("addDetails") = false);
        m.def("Help", &PyHelp, "Show basic help information");
        m.def("StartRenderer", &PyStartOpenGLRenderer, "Start OpenGL rendering engine (in separate thread) for visualization of rigid or flexible multibody system; use verbose=1 to output information during OpenGL window creation; verbose=2 produces more output and verbose=3 gives a debug level; some of the information will only be seen in windows command (powershell) windows or linux shell, but not inside iPython of e.g. Spyder", py::arg("verbose") = 0);
        m.def("IsRendererActive", &PyIsRendererActive, "returns True if GLFW renderer is available and running; otherwise False");
        ...
        m.def("Go", &PythonGo, "Creates a SystemContainer SC and a main multibody system mbs");
    ...
    py::class_<MainSystem>(m, "MainSystem")
        .def(py::init(&MainSystem::ForbidConstructor))
...
//        LTG readout functions:
    ...
    py::class_<PyGeneralContact>(m, "GeneralContact")
        ...
        .def("GetMarkerBasedSphere", &PyGeneralContact::PyGetMarkerBasedSphere, "Get dictionary with position, radius and markerIndex for markerBasedSphere index, as returned e.g. from GetItemsInBox", py::arg("localIndex"))
