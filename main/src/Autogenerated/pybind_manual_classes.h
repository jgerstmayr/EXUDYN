...
//        LTG readout functions:
    ...
    py::class_<PyGeneralContact>(m, "GeneralContact")
        ...
        .def("FinalizeContact", &PyGeneralContact::PyFinalizeContact, "WILL CHANGE IN FUTURE: Call this function after mbs.Assemble(); precompute some contact arrays (mainSystem needed) and set up necessary parameters for contact: friction, SearchTree, etc.; done after all contacts have been added; function performs checks; empty box will autocompute size!", py::arg("mainSystem"), py::arg("searchTreeSize"), py::arg("frictionPairingsInit"), py::arg("searchTreeBoxMin") = (std::vector<Real>)Vector3D( EXUstd::MAXREAL ), py::arg("searchTreeBoxMax") = (std::vector<Real>)Vector3D( EXUstd::LOWESTREAL ))
        .def("AddSphereWithMarker", &PyGeneralContact::AddSphereWithMarker, "add contact object using a marker (Position or Rigid), radius and contact/friction parameters; contact is possible between spheres (circles in 2D) (if intraSphereContact = true) and between sphere (=circle) and ANCFCable2D", py::arg("markerIndex"), py::arg("radius"), py::arg("contactStiffness"), py::arg("contactDamping"), py::arg("frictionMaterialIndex"))
