...
// AUTO:  last modified = 2022-06-16
// AUTO:  ++++++++++++++++++++++

    //++++++++++++++++++++++++++++++++
    py::class_<PyBeamSection>(m, "BeamSection", "BeamSection class") // AUTO: 
        .def(py::init<>())
        .def_property("dampingMatrix", &PyBeamSection::PyGetDampingMatrix, &PyBeamSection::PySetDampingMatrix, py::return_value_policy::reference)
        .def_property("inertia", &PyBeamSection::PyGetInertia, &PyBeamSection::PySetInertia, py::return_value_policy::reference)
        .def_property("massPerLength", &PyBeamSection::PyGetMassPerLength, &PyBeamSection::PySetMassPerLength, py::return_value_policy::reference)
        .def_property("stiffnessMatrix", &PyBeamSection::PyGetStiffnessMatrix, &PyBeamSection::PySetStiffnessMatrix, py::return_value_policy::reference)
    ...
    py::class_<NewtonSettings>(m, "NewtonSettings", "NewtonSettings class") // AUTO: 
        ...
        .def_readwrite("adaptInitialResidual", &NewtonSettings::adaptInitialResidual, "flag (true/false); false = standard; True: if initialResidual is very small (or zero), it may increas dramatically in first step; to achieve relativeTolerance, the initialResidual will by updated by a higher residual within the first Newton iteration")
        ...
        .def_readwrite("modifiedNewtonJacUpdatePerStep", &NewtonSettings::modifiedNewtonJacUpdatePerStep, "True: compute Jacobian at every time step, but not in every iteration (except for bad convergence ==> switch to full Newton)")
        ...
        .def_readwrite("useModifiedNewton", &NewtonSettings::useModifiedNewton, "True: compute Jacobian only at first step; no Jacobian updates per step; False: Jacobian computed in every step")
    ...
    py::class_<ExplicitIntegrationSettings>(m, "ExplicitIntegrationSettings", "ExplicitIntegrationSettings class") // AUTO: 
        ...
        .def_readwrite("useLieGroupIntegration", &ExplicitIntegrationSettings::useLieGroupIntegration, "True: use Lie group integration for rigid body nodes; must be turned on for Lie group nodes, but also improves integration of other rigid body nodes. Only available for RK44 integrator.")
    ...
    py::class_<Parallel>(m, "Parallel", "Parallel class") // AUTO: 
        ...
        .def_readwrite("parallelizeJacobians", &Parallel::parallelizeJacobians, "(not available yet) compute jacobians multi-threaded (only makes sense for medium to large systems!)")
        .def_readwrite("parallelizeMassMatrix", &Parallel::parallelizeMassMatrix, "(not available yet) compute mass matrix multi-threaded (only makes sense for medium to large systems!)")
        .def_readwrite("parallelizeResiduals", &Parallel::parallelizeResiduals, "(not available yet) compute RHS vectors of objects and connectors and loads multi-threaded (only makes sense for medium to large systems!)")
        .def_readwrite("stopThreadsInSerialSections", &Parallel::stopThreadsInSerialSections, "(not available yet) for large scale problems, if steps take longer than 5 ms, parallel threads are stopped in serial regions (etc. for solver) to improve overall performance")
        .def_readwrite("useMTforSolver", &Parallel::useMTforSolver, "(not available yet) use multi-threaded optimized vector, vector-matrix and matrix-matrix operations for solver and system operations (only makes sense for medium to large systems!)")
        .def_readwrite("useSIMDforSolver", &Parallel::useSIMDforSolver, "(not available yet) use AVX optimized vector, vector-matrix and matrix-matrix operations for solver and system operations (may already speedup for 16 coordinates)")
    ...
    py::class_<VSettingsGeneral>(m, "VSettingsGeneral", "VSettingsGeneral class") // AUTO: 
        .def(py::init<>())
        .def_readwrite("autoFitScene", &VSettingsGeneral::autoFitScene, "automatically fit scene within first second after StartRenderer()")
        ...
        .def_readwrite("useWindowsMonitorScaleFactor", &VSettingsGeneral::useWindowsMonitorScaleFactor, "the windows monitor scaling is used for increased visibility of texts on high resolution monitors; based on GLFW glfwGetWindowContentScale")
    ...
    py::class_<VSettingsContact>(m, "VSettingsContact", "VSettingsContact class") // AUTO: 
        ...
        .def_readwrite("showBoundingBoxes", &VSettingsContact::showBoundingBoxes, "show bounding boxes of all GeneralContacts")
        ...
        .def_readwrite("showSearchTree", &VSettingsContact::showSearchTree, "show search tree of all GeneralContacts")
        .def_readwrite("showSearchTreeCells", &VSettingsContact::showSearchTreeCells, "show cells inside search tree")
    ...
    py::class_<VSettingsOpenGL>(m, "VSettingsOpenGL", "VSettingsOpenGL class") // AUTO: 
        ...
        .def_readwrite("drawNormalsLength", &VSettingsOpenGL::drawNormalsLength, "length of normals; used for debugging")
        ...
        .def_readwrite("initialMaxSceneSize", &VSettingsOpenGL::initialMaxSceneSize, "initial maximum scene size (auto: diagonal of cube with maximum scene coordinates); used for 'zoom all' functionality and for visibility of objects; overwritten if autoFitScene = True")
        ...
        .def_readwrite("initialZoom", &VSettingsOpenGL::initialZoom, "initial zoom of scene; overwritten/ignored if autoFitScene = True")
        ...
        .def_readwrite("lightModelTwoSide", &VSettingsOpenGL::lightModelTwoSide, "enlighten also backside of object; maps to OpenGL glLightModeli(GL_LIGHT_MODEL_TWO_SIDE,...)")
        .def_readwrite("lineSmooth", &VSettingsOpenGL::lineSmooth, "draw lines smooth")
        .def_readwrite("lineWidth", &VSettingsOpenGL::lineWidth, "width of lines used for representation of lines, circles, points, etc.")
        ...
        .def_readwrite("textLineWidth", &VSettingsOpenGL::textLineWidth, "width of lines used for representation of text")
    ...
    py::class_<VSettingsExportImages>(m, "VSettingsExportImages", "VSettingsExportImages class") // AUTO: 
        ...
        .def_readwrite("saveImageFormat", &VSettingsExportImages::saveImageFormat, "format for exporting figures: currently only PNG and TGA available; PNG is not available for Ubuntu18.04 (check  use TGA has highest compatibility with all platforms")
    ...
    py::class_<SolverOutputData>(m, "SolverOutputData", "SolverOutputData class") // AUTO: 
        ...
        .def_readwrite("finishedSuccessfully", &SolverOutputData::finishedSuccessfully, "flag is false until solver finshed successfully (can be used as external trigger)")
        ...
        .def_readwrite("sensorValuesTemp", &SolverOutputData::sensorValuesTemp, "temporary vector for per sensor values (overwritten for every sensor; usually contains last sensor values)")
        .def_readwrite("sensorValuesTemp2", &SolverOutputData::sensorValuesTemp2, "additional temporary vector for per sensor values (overwritten for every sensor; usually contains time+last sensor values)")
    ...
    py::class_<MainSolverStatic>(m, "MainSolverStatic", "MainSolverStatic class") // AUTO: 
        ...
        .def("ComputeJacobianAE", &MainSolverStatic::ComputeJacobianAE, "add jacobian of algebraic equations (multiplied with factor) to systemJacobian in cSolver; the scalarFactors are scaling the derivatives w.r.t. ODE2 coordinates, ODE2_t (velocity) coordinates and ODE1 coordinates; if velocityLevel == true, the constraints are evaluated at velocity level", py::arg("mainSystem"), py::arg("scalarFactor_ODE2") = 1., py::arg("scalarFactor_ODE2_t") = 0., py::arg("scalarFactor_ODE1") = 1., py::arg("velocityLevel") = false)
        .def("ComputeJacobianODE1RHS", &MainSolverStatic::ComputeJacobianODE1RHS, "add jacobian (multiplied with factors for ODE2 and ODE1 coordinates) of ODE1RHS to systemJacobian in cSolver", py::arg("mainSystem"), py::arg("scalarFactor_ODE2") = 1., py::arg("scalarFactor_ODE2_t") = 0., py::arg("scalarFactor_ODE1") = 1.)
        .def("ComputeJacobianODE2RHS", &MainSolverStatic::ComputeJacobianODE2RHS, "set systemJacobian to zero and add jacobian (multiplied with factors for ODE2 and ODE1 coordinates) of ODE2RHS to systemJacobian in cSolver", py::arg("mainSystem"), py::arg("scalarFactor_ODE2") = 1., py::arg("scalarFactor_ODE2_t") = 0., py::arg("scalarFactor_ODE1") = 1.)
    ...
    py::class_<MainSolverImplicitSecondOrder>(m, "MainSolverImplicitSecondOrder", "MainSolverImplicitSecondOrder class") // AUTO: 
        ...
        .def("ComputeJacobianAE", &MainSolverImplicitSecondOrder::ComputeJacobianAE, "add jacobian of algebraic equations (multiplied with factor) to systemJacobian in cSolver; the scalarFactors are scaling the derivatives w.r.t. ODE2 coordinates, ODE2_t (velocity) coordinates and ODE1 coordinates; if velocityLevel == true, the constraints are evaluated at velocity level", py::arg("mainSystem"), py::arg("scalarFactor_ODE2") = 1., py::arg("scalarFactor_ODE2_t") = 1., py::arg("scalarFactor_ODE1") = 1., py::arg("velocityLevel") = false)
        .def("ComputeJacobianODE1RHS", &MainSolverImplicitSecondOrder::ComputeJacobianODE1RHS, "add jacobian (multiplied with factors for ODE2 and ODE1 coordinates) of ODE1RHS to systemJacobian in cSolver", py::arg("mainSystem"), py::arg("scalarFactor_ODE2") = 1., py::arg("scalarFactor_ODE2_t") = 0., py::arg("scalarFactor_ODE1") = 1.)
        .def("ComputeJacobianODE2RHS", &MainSolverImplicitSecondOrder::ComputeJacobianODE2RHS, "set systemJacobian to zero and add jacobian (multiplied with factors for ODE2 and ODE1 coordinates) of ODE2RHS to systemJacobian in cSolver", py::arg("mainSystem"), py::arg("scalarFactor_ODE2") = 1., py::arg("scalarFactor_ODE2_t") = 1., py::arg("scalarFactor_ODE1") = 1.)
