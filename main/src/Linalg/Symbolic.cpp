...
#define PyUFobject pybind11::object

template <typename UFT>
PythonUserFunctionBase<UFT>::~PythonUserFunctionBase()
{
	delete pyObject;
}

template <typename UFT>
PythonUserFunctionBase<UFT>::PythonUserFunctionBase() : pyObject(new PyUFobject()), ufType(UserFunctionType::_None)
{
	*pyObject = py::cast((Index)0);
}

template <typename UFT>
void PythonUserFunctionBase<UFT>::SetPythonObject(const PyUFobject& pyObjectInit)
{
	if (py::isinstance<py::dict>(pyObjectInit))
	{
		py::dict pyDict = py::cast<py::dict>(pyObjectInit);
		CHECKandTHROW(pyDict.contains("function") && pyDict.contains("type"), "PythonUserFunctionBase::SetPythonObject: invalid function dictionary");
	...
	else if (py::isinstance<py::function>(pyObjectInit)) //old-style or first initialization
	{
		*pyObject = pyObjectInit;
		ufType = UserFunctionType::Python;
		//userFunction = py::cast< std::function<bool(const MainSystem & mainSystem, Real t)> >(pyObjectInit);
		userFunction = py::cast<UFT>(pyObjectInit);
	}
	else if (EPyUtils::IsPyTypeInteger(pyObjectInit))
	{
...
//! set Python user function using PySymbolicFunction
template <typename UFT>
void PythonUserFunctionBase<UFT>::SetPythonUserFunction(const PyUFobject& pyObjectInit)
{
	if (py::isinstance<Symbolic::PySymbolicUserFunction>(pyObjectInit))
	{
		const auto& symUF = py::cast<Symbolic::PySymbolicUserFunction>(pyObjectInit);
		userFunction = symUF.GetSTDfunction<UFT>();
	...
	{
		SetPythonObject(pyObjectInit); //<UFT>
	}
}

////! copy assignment operator
//template <typename UFT>
//PythonUserFunctionBase<UFT>& PythonUserFunctionBase<UFT>::operator=(const PythonUserFunctionBase<UFT>& other)
//{
//	if (this == &other) { return *this; }
//
//	userFunction = other.userFunction;
//	ufType = other.ufType;
...
template <typename UFT>
PythonUserFunctionBase<UFT>::operator pybind11::object() const {
...
template <typename UFT>
PythonUserFunctionBase<UFT>& PythonUserFunctionBase<UFT>::operator=(const pybind11::object& pyObjectInit)
{
	SetPythonUserFunction(pyObjectInit);
	return *this;
}

template <typename UFT>
void PythonUserFunctionBase<UFT>::Reset()
{
	SetPythonObject(py::cast((Index)0));
}

template <typename UFT>
PyUFobject PythonUserFunctionBase<UFT>::GetPythonDictionary() const
{
	switch (ufType)
	{
	case UserFunctionType::_None:
	{
		//CHECKandTHROW(py::isinstance<py::int_>(*pyObject) && py::cast<Index>(*pyObject) == 0, "PythonUserFunctionBase::GetPythonDictionary: 0 expected!");
	...
	default:
		CHECKandTHROWstring("PythonUserFunctionBase::GetPythonDictionary(): failed: invalid function type");
	}
	return py::cast((Index)(-1));
}

#include "Autogenerated/PythonUserFunctionsTemplates.h"
...
void Init_Pybind_Symbolic(py::module& m) {
	...
	py::class_<Symbolic::SReal>(m, "Real", "symbolic.Real: a hybrid Real-symbolic type used for recording of expression trees")
		...
		//+++++++++++++++++++++++++++++++++++++++++++
		.def("SetValue", &Symbolic::SReal::SetSymbolicValue<Real>,
			"set value of symbolic Real quantity")

		.def("SetValue", &Symbolic::SReal::SetSymbolicValue<Symbolic::SReal>,
			"set value of symbolic Real quantity")
	...
	py::class_<Symbolic::PySymbolicUserFunction>(m, "UserFunction", "used to create symbolic user functions, evaluated purly on C++ side")
		...
		.def("SetUserFunctionFromDict", &Symbolic::PySymbolicUserFunction::SetUserFunctionFromDict,
			"set user function given by according dictionary, item index and user function name")
		//.def("TransferUserFunction2Item", &Symbolic::PySymbolicUserFunction::TransferUserFunction2Item,
...
void PyTest_unused()
{
	//std::cout << "Size of empty std::function: " << sizeof(std::function<void()>) << " bytes" << std::endl;
