...
Mfem = ScipySparseCSRtoCSR( fem.GetMassMatrix(sparse=True) )
Kfem = ScipySparseCSRtoCSR( fem.GetStiffnessMatrix(sparse=True) )
Mcsr = CSRtoScipySparseCSR(Mfem)
Kcsr = CSRtoScipySparseCSR(Kfem)

Mmat = exu.MatrixContainer()
Mmat.SetWithSparseMatrixCSR(nRows,nRows, Mfem, useDenseMatrix=False)
Kmat = exu.MatrixContainer()
Kmat.SetWithSparseMatrixCSR(nRows,nRows,Kfem, useDenseMatrix=False)

#only for test purpose, stiffness-proportional damping:
Dmat = exu.MatrixContainer()
Dfem = copy.copy(Kfem)
Dfem[:,2] *= 5e-3 #this is the damping factor alpha, using D=alpha*K
Dcsr = CSRtoScipySparseCSR(Dfem)
Dmat.SetWithSparseMatrixCSR(nRows,nRows, Dfem, useDenseMatrix=False)

#create copy of matrix, only used for jacobian, as we need to multiply with fODE2 factor:
KDfem = copy.copy(Kfem)
...
def UFjacobian(mbs, t, itemNumber, c, c_t, fODE2, fODE2_t):
    ...
    KDfem[:,2] = fODE2 * Kfem[:,2] + fODE2_t * Dfem[:,2]
    jac.SetWithSparseMatrixCSR(nRows,nRows,KDfem,useDenseMatrix=False) 
    return jac
    
testUserFunction = True #set False to switcht to (faster) case without user function
