...
if __name__ == '__main__': #only main thread for multiprocessing Pool
    ...
    #parameters
    meshSize = 0.005 #standard: 0.005; 0.0011: memory limit (96GB) for NGsolve; < 0.0015 makes problems with scipy eigensolver
    showStresses = False #may take very long for large number of modes/nodes
    ...
    if True:
        ...
        if showStresses:
            print("ComputePostProcessingModes femCrank ... (may take a while)")
            start_time = time.time()
        ...
        if showStresses:
            print("ComputePostProcessingModes femConrod ... (may take a while)")
            start_time = time.time()
        ...
        if showStresses:
            print("ComputePostProcessingModes femPiston ... (may take a while)")
            start_time = time.time()
            femPiston.ComputePostProcessingModes(material=mat, 
                                           outputVariableType=varType,
                                           numberOfThreads=-1)
            print("--- %s seconds ---" % (time.time() - start_time))
