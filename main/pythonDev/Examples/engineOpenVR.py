...
for engine in engines:
    ...
    # SC.visualizationSettings.openGL.initialZoom = 0.4*1 #0.4*max scene size
    SC.visualizationSettings.window.startupTimeout = 100000 #wait ms for SteamVR
    SC.visualizationSettings.window.renderWindowSize=[1176, 1320]
    ...
    SC.visualizationSettings.interactive.openVR.actionManifestFileName = "C:/DATA/cpp/EXUDYN_git/main/pythonDev/Examples/testData/vrActions.json"
    #good for openVR
    ...
    if not createFigures: 
        exu.StartRenderer()
        if 'renderState' in exu.sys:
            SC.SetRenderState(exu.sys[ 'renderState' ])
    
    if not createFigures: mbs.WaitForUserToContinue()
    exu.SolveDynamic(mbs, simulationSettings)
    
    SC.WaitForRenderEngineStopFlag()
    if not createFigures: exu.StopRenderer() #safely close rendering window!
