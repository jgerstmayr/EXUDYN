...
.. _examples-rosexampleturtle:

*******************
ROSExampleTurtle.py
*******************

You can view and download this file on Github: `ROSExampleTurtle.py <https://github.com/jgerstmayr/EXUDYN/tree/master/main/pythonDev/Examples/ROSExampleTurtle.py>`_
...
.. code-block:: python
   ...
   #           install ROS (ROS1 noetic) including rospy (see exudynROSInterface.py)
   #           prerequisite to use: 
   ...
   #               python3 ROSexampleTurtle.py -pub
   #           start example with argument -noTrack, to not track the turtle:
   #               python3 ROSexampleTurtle.py -noTrack
   #           for even more ROS functionality create a ROS package (e.q. myExudynInterface) in a catkin workspace, 
   #           copy files ROSExampleTurtle.py, Turtle.stl and ROSExampleBringup.launch file in corresponding folders within the package
   ...
   def main():
       ...
       try:
           ...
           stlGrafics = GraphicsDataFromSTLfile(path2stl+'Turtle.stl',color=[1,0,0,1],scale=0.25,pOff=[0.35,0,0], Aoff=turtleRot)
