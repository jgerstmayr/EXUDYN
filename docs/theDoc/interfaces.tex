...
\begin{center}
  ...
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    ...
    lieGroupSimplifiedKinematicRelations &     \tabnewline bool &      &     False &     True: for Lie group nodes, the integrator uses the original kinematic relations of the Bruls and Cardona 2010 paper; False (recommended): higher accuracy as proposed in paper by Holzinger, Arnold, Gerstmayr, sigma-modified Lie group generalized alpha methods for constrained multibody systems, 2025 (to be sumitted)\\ \hline
    ...
    storeInitialAlgebraicCoordinates &     \tabnewline bool &      &     True &     True: IF computeInitialAccelerations=True, store initial algebraic coordinates (usually the Lagrange multipliers) in the initial coordinates vector (and thus in the first line of the coordinates solution file); for further details on limitations, see computeInitialAccelerations\\ \hline
...
\begin{center}
  ...
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    ...
    numberOfSteps &     PReal &      &     100 &     $n\_{steps}$: number of steps in time integration; (maximum) stepSize $h$ is computed from $h = \frac{t\_{end} - t\_{start}}{n\_{steps}}$; for automatic stepsize control, this stepSize is the maximum steps size, $h\_{max} = h$; numberOfSteps can also be a float type, but must be close to an integer (relative tolerance $100\cdot\varepsilon$) as it is silently rounded to int\\ \hline
...
\begin{center}
  ...
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    ...
    boundingBoxZoomAllFactor &     PFloat &      &     1.2 &     factor on boundingBox for zoom all (without minimum offset)\\ \hline
    boundingBoxZoomAllOffset &     UFloat &      &     0.01 &     minimum offset to bounding box of scene in window - width or height, whatever is smaller; adjust for very small or large scenes; may be negative\\ \hline
    circleTiling &     PInt &      &     16 &     global number of segments for circles; if smaller than 2, 2 segments are used (flat)\\ \hline
    coordinateSystemSize &     PFloat &      &     5. &     size of coordinate system relative to font size\\ \hline
    cylinderTiling &     PInt &      &     16 &     global number of segments for cylinders; if smaller than 2, 2 segments are used (flat)\\ \hline
    graphicsUpdateInterval &     UFloat &      &     0.1 &     interval of graphics update during simulation in seconds; 0.1 = 10 frames per second; low numbers might slow down computation speed\\ \hline
    limitWindowToScreenSize &     bool &      &     True &     True: size for render window of respective view is limited to screen size; False: larger window sizes (e.g. for rendering) allowed according to renderWindowSize\\ \hline
    ...
    minSceneSize &     PFloat &      &     0.1 &     minimum scene size for initial scene size and for autoFitScene, to avoid division by zero; SET GREATER THAN ZERO\\ \hline
    pointSize &     PFloat &      &     0.01 &     global point size (absolute)\\ \hline
    ...
    rendererStartupTimeout &     PInt &      &     2500 &     OpenGL render windows startup timeout in ms (change might be necessary if CPU is very slow)\\ \hline
    ...
    showHelpOnStartup &     UInt &      &     5 &     seconds to show help message on startup (0=deactivate)\\ \hline
    ...
    zoomAllUseBoundingBox &     bool &      &     True &     if true, use exact scene bounding box (but not including texts) for zoom; does not include perspective effects!\\ \hline
	  \end{longtable}
	\end{center}
...
\mysubsubsubsection{VSettingsContourAdvanced}
\label{sec:VSettingsContourAdvanced}
Advanced settings for contour plots.

\noindent VSettingsContourAdvanced has the following items:
%reference manual TABLE
\begin{center}
  \footnotesize
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    ...
    contourColor0 &     Float4 &     4 &     [0.1,0.1,0.9,1.] &     \tabnewline RGBA color for relative value 0 used for contour plot; alpha is ignored\\ \hline
    contourColor1 &     Float4 &     4 &     [0.1,0.9,0.9,1.] &     \tabnewline RGBA color for relative value 0.25 used for contour plot; alpha is ignored\\ \hline
    contourColor2 &     Float4 &     4 &     [0.1,0.9,0.1,1.] &     \tabnewline RGBA color for relative value 0.25 used for contour plot; alpha is ignored\\ \hline
    contourColor3 &     Float4 &     4 &     [0.9,0.9,0.1,1.] &     \tabnewline RGBA color for relative value 0.25 used for contour plot; alpha is ignored\\ \hline
    contourColor4 &     Float4 &     4 &     [0.9,0.1,0.1,1.] &     \tabnewline RGBA color for relative value 0.25 used for contour plot; alpha is ignored\\ \hline
    contourColorMax &     Float4 &     4 &     [0.9,0.9,0.9,1.] &     \tabnewline RGBA color if relative value in contour plot is larger than 1 (if automaticRange=False); alpha is ignored\\ \hline
    contourColorMin &     Float4 &     4 &     [0.1,0.1,0.1,1.] &     \tabnewline RGBA color if relative value in contour plot is smaller than 0 (if automaticRange=False); alpha is ignored\\ \hline
...
\begin{center}
  ...
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    \hline
    \bf Name & \bf type / function return type & \bf size & \bf default value / function args & \bf description \\ \hline
    advanced &     VSettingsContourAdvanced &      &      &     advanced settings for contour\\ \hline
...
\begin{center}
  ...
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    ...
    crossSectionFilled &     bool &      &     True &     if implemented for element, cross section is drawn as solid (filled) instead of wire-frame; NOTE: some quantities may not be interpolated correctly over cross section in visualization; equivalent to drawSolid of shells\\ \hline
    ...
    drawVerticalFactor &     UFloat &      &     1. &     factor for outputVariable to be drawn along cross section (vertically)\\ \hline
    ...
    reducedAxialInterploation &     bool &      &     True &     if True, the interpolation along the beam axis may be lower than the beam element order; this may, however, show more consistent values than a full interpolation, e.g. for strains or forces\\ \hline
	  \end{longtable}
	\end{center}
...
\mysubsubsubsection{VSettingsShells}
\label{sec:VSettingsShells}
Visualization settings for plate/shell finite elements.

\noindent VSettingsShells has the following items:
%reference manual TABLE
\begin{center}
  \footnotesize
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    \hline
    \bf Name & \bf type / function return type & \bf size & \bf default value / function args & \bf description \\ \hline
    drawSolid &     bool &      &     True &     if true: to draw plates/shells as 3D objects; false: only the element surface is drawn; equivalent to crossSectionFilled in beams\\ \hline
    thicknessFactor &     PFloat &      &     1. &     a factor multiplied with the thickness of shells/plates only for visualization (e.g. to make some effects more visible)\\ \hline
	  \end{longtable}
	\end{center}
...
\begin{center}
  ...
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    ...
    shells &     VSettingsShells &      &      &     visualization settings for plates and shells\\ \hline
...
\mysubsubsubsection{VSettingsCamera}
\label{sec:VSettingsCamera}
...
\noindent VSettingsCamera has the following items:
%reference manual TABLE
\begin{center}
  \footnotesize
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    \hline
    \bf Name & \bf type / function return type & \bf size & \bf default value / function args & \bf description \\ \hline
    cameraPosition &     Float3 &     3 &     [0.,0.,0.] &     \tabnewline if modelCentricView=True: offset to camera position in model view (and, if used, relative to tracked marker - instead of a tracked marker position, you could also just change the camera position in camera-centric views); camera rotation follows modelRotation in renderState\\ \hline
    clippingPlaneDistance &     float &      &     0. &     distance of clipping plane on normal vector; see also clippingPlaneNormal and openGL.advanced.clippingPlaneColor\\ \hline
    clippingPlaneNormal &     Float3 &     3 &     [0.,0.,0.] &     \tabnewline normal vector of clipping plane, e.g. [0,0,1] to set a xy-clipping plane; the clipped half-space is in direction of the normal; use [0,0,0] to deactivate clipping plane; Note that clipping is mainly made for triangles in order to visualize hidden objects and currently it only fully clips triangles, but does not exactly cut them; see also clippingPlaneDistance and openGL.advanced.clippingPlaneColor\\ \hline
    modelCentricView &     bool &      &     True &     True: rotations and translations are applied to model, while camera stays far enough away from the model and always captures the whole model (everything is in front of camera plane); False: camera moves and rotates while model stays in physical space; only geometry in front of camera is visible; note that the behavior of trackMarker changes with modelCentricView and some features are not available in case of modelCentricView=False.\\ \hline
    nearFarPlaneOffset &     Float3 &     3 &     [0.,0.,0.] &     \tabnewline the three values are [nearPlaneOffset, farPlaneOffset, flag]; if flag=0, the offsets are ignored and computed automatically, using x = 2 * maxSceneSize * zMaxSceneFactor, setting near plane to -x and far plane to +x in case of modelCentricView=True and setting near plane to 0.01 (minimal offset to eye point) and far plane to +x if modelCentricView=False; if flag=1, the near and far plane values are just overwritten; note that positive values for near plane make objects in front of the camera invisible while negative values make objects behind the camera plane visible; in case of camera-centric view, the eyepoint can be shifted backwards using cameraPosition accordingly.\\ \hline
    perspective &     UFloat &      &     0. &     parameter prescribes amount of perspective (0=no perspective=orthographic projection; positive values increase perspective; feasible values are 0.001 (little perspective) ... 1 (extreme: 5), where larger values are possible but should be used with care; NOTE that the relation to the common field of view (FOV) angle alpha, with alpha=90°, is given by perspective = tan(alpha/2) = 1; mouse coordinates (F3) can not be shown with perspective>0\\ \hline
    ...
    useRaytracer &     bool &      &     False &     True: use (software) raytracer for this view; False: use standard OpenGL renderer\\ \hline
	  \end{longtable}
	\end{center}
...
\mysubsubsubsection{VSettingsScene}
\label{sec:VSettingsScene}
...
\noindent VSettingsScene has the following items:
%reference manual TABLE
\begin{center}
  \footnotesize
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    \hline
    \bf Name & \bf type / function return type & \bf size & \bf default value / function args & \bf description \\ \hline
    drawCoordinateSystem &     UInt &      &     2 &     0 = no coordinate system shown, 1 = draw lines with text, 2 = draw arrows, 3 = draw arrows with text\\ \hline
    ...
    showFaceEdges &     bool &     1 &     False &     True: show edges of triangles; using the options showFaces=false and showFaceEdges=true gives are wire frame representation\\ \hline
    showFaces &     bool &     1 &     True &     True: show faces of triangles, etc.; using the options showFaces=false and showFaceEdges=true gives are wireframe representation\\ \hline
    showLines &     bool &     1 &     True &     True: show lines (other lines than face and mesh edges)\\ \hline
    showMeshEdges &     bool &     1 &     True &     True: show edges of finite elements; independent of showFaceEdges\\ \hline
    showMeshFaces &     bool &     1 &     True &     True: show faces of finite elements; independent of showFaces\\ \hline
    worldBasisSize &     PFloat &      &     1.0 &     size of world basis coordinate system\\ \hline
	  \end{longtable}
	\end{center}
...
\begin{center}
  ...
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    \hline
    \bf Name & \bf type / function return type & \bf size & \bf default value / function args & \bf description \\ \hline
    alwaysOnTop &     bool &      &     False &     True: render window of respective view will be always on top of all other windows\\ \hline
    globalFontSize &     PFloat &      &     12. &     general text font size (roughly measured in pixels); if useWindowsDisplayScaleFactor=True, the the textSize is multplied with the windows display scaling (monitor scaling; content scaling) factor for larger texts on on high resolution displays; for bitmap fonts, the maximum size of any font (standard/large/huge) is limited to 256 (which is not recommended, especially if you do not have a powerful graphics card)\\ \hline
    lockModelView &     bool &      &     False &     True: all movements (with mouse/keys), rotations, zoom are disabled; the view is either based on initial values (or on the current state) ==> initial zoom, rotation and center point need to be adjusted, approx. 0.4*maxSceneSize is a good value\\ \hline
    maximize &     bool &      &     False &     True: render window of respective view will be maximized at startup\\ \hline
    renderWindowSize &     Index2 &     2 &     [1024,768] &     initial size of render window of respective view for specific view in pixels for\\ \hline
    showComputationInfo &     bool &      &     True &     true = show (hide) all computation information including Exudyn and version\\ \hline
    showMouseCoordinates &     bool &      &     False &     True: show OpenGL coordinates and distance to last left mouse button pressed position in renderer status message; switched on/off with key 'F3'; only works for axis-aligned ortho-projections\\ \hline
    showRenderStateInfo &     bool &      &     False &     True: show renderer.state infos regarding zoom, offset and rotation in renderer status message; switched on/off with 'CTRL-F3'\\ \hline
    showWindow &     bool &      &     True &     True: render window of respective view is shown when created; False: window will be iconified when created (e.g. if you are starting multiple computations automatically)\\ \hline
	  \end{longtable}
	\end{center}
...
\mysubsubsubsection{VSettingsView}
\label{sec:VSettingsView}
Settings for view including camera, scene, window, and advanced options to setup a view or view window.

\noindent VSettingsView has the following items:
%reference manual TABLE
\begin{center}
  \footnotesize
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    \hline
    \bf Name & \bf type / function return type & \bf size & \bf default value / function args & \bf description \\ \hline
    camera &     VSettingsCamera &      &      &     settings for camera like perspective, marker tracking or clipping plane\\ \hline
    scene &     VSettingsScene &      &      &     settings which change scene representation, showing edges, faces or world basis\\ \hline
    window &     VSettingsWindow &      &      &     visualization settings for window that are individual to each view\\ \hline
	  \end{longtable}
	\end{center}
...
\mysubsubsubsection{VSettingsWindowDeprecated}
\label{sec:VSettingsWindowDeprecated}
...
\noindent VSettingsWindowDeprecated has the following items:
%reference manual TABLE
\begin{center}
  \footnotesize
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
...
\begin{center}
  ...
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    ...
    multiThreadedDialogs &     bool &      &     True &     True: During dialogs, the OpenGL render windows will still get updates of changes in dialogs, etc., which may cause problems on some platforms or for some (complicated) models; False: changes of dialogs will take effect when dialogs are closed\\ \hline
...
\mysubsubsubsection{VSettingsRaytracerAdvanced}
\label{sec:VSettingsRaytracerAdvanced}
Advanced settings for raytracer.

\noindent VSettingsRaytracerAdvanced has the following items:
%reference manual TABLE
\begin{center}
  \footnotesize
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
...
\begin{center}
  ...
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    \hline
    \bf Name & \bf type / function return type & \bf size & \bf default value / function args & \bf description \\ \hline
    advanced &     VSettingsRaytracerAdvanced &      &      &     advanced settings for raytracer\\ \hline
    ...
    globalFogDensity &     UFloat &      &     0. &     global fog density; fog is deactivated if fogDensity=0, otherwise it is a density relative to scene max size; as it is relative, the factor has to be relatively high to be visible (usually >1)\\ \hline
    ...
    lightRadiusVariations &     PInt &     1 &     1 &     if lightRadiusVariations>1, this defines the number of positions that are used to compute the effect of distributed lights (larger is slower but better quality); range=1..256; avoid squares of integers; good values: 1 (hard shadow boundaries), 6, 13, 20, 31, 72, 130, 240; for lower values, use shadowSmoothingSteps=2..8\\ \hline
    ...
    multiSampling &     PInt &     1 &     1 &     Multi-sampling used for rendering of faces, lines and text; increases image quality along edges (lines, etc.) but INCREASES rendering costs dramatically (multiSampling=3 => 3x3=9 times slower); also used for shadow if shadowScalingFactor=0; values only accepted in range [1..4]\\ \hline
    ...
    verbose &     Index &      &     0 &     1: print out some debug information on rendering, in particular rendering timings and counter; 2 and higher: advanced debug information\\ \hline
	  \end{longtable}
	\end{center}
...
\mysubsubsubsection{VSettingsOpenGLAdvanced}
\label{sec:VSettingsOpenGLAdvanced}
Advanced settings for openGL.

\noindent VSettingsOpenGLAdvanced has the following items:
%reference manual TABLE
\begin{center}
  \footnotesize
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    ...
    faceNormalsColor &     Float4 &     4 &     [0.8,0.2,0.2,1.] &     \tabnewline global RGBA color for face normals\\ \hline
    initialCenterPoint &     Float3 &     3 &     [0.,0.,0.] &     \tabnewline centerpoint of scene (3D) at renderer startup; overwritten if autoFitScene = True; only used in case that modelCentricView=True\\ \hline
    ...
    initialModelRotation &     StdArray33F &     3x3 &     [Matrix3DF[3,3,1.,0.,0., 0.,1.,0., 0.,0.,1.]] &     \tabnewline initial model rotation matrix for OpenGl; in python use e.g.: initialModelRotation=[[1,0,0],[0,1,0],[0,0,1]]; only used in case that modelCentricView=True\\ \hline
    initialZoom &     UFloat &      &     1. &     initial zoom of scene; overwritten/ignored if autoFitScene = True\\ \hline
    lightModelLocalViewer &     bool &     1 &     False &     True: the camera origin is used to compute shininess effects (more realistic); maps to OpenGL glLightModeli(GL\_LIGHT\_MODEL\_LOCAL\_VIEWER,...)\\ \hline
    ...
    polygonOffset &     float &      &     0.05 &     general polygon offset for polygons, except for shadows; use this parameter to draw polygons behind lines to reduce artifacts for very large or small models\\ \hline
    ...
    showBoundingBox &     bool &     1 &     False &     show scene bounding box (red), as available in renderState.boundingBox; NOTE that the bounding box is only updated with ZoomAll or at startup; this is a debug flag and it may show reasongs for strange ZoomAll behavior, as ZoomAll should zoom to the bounding box; does only work for perspective=0\\ \hline
    ...
    vertexNormalsColor &     Float4 &     4 &     [0.8,0.2,0.2,1.] &     \tabnewline global RGBA color for vertex normals\\ \hline
	  \end{longtable}
	\end{center}
...
\mysubsubsubsection{VSettingsLight}
\label{sec:VSettingsLight}
Settings for lights.

\noindent VSettingsLight has the following items:
%reference manual TABLE
\begin{center}
  \footnotesize
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    \hline
    \bf Name & \bf type / function return type & \bf size & \bf default value / function args & \bf description \\ \hline
    constantAttenuation &     float &     1 &     1.0 &     constant attenuation coefficient of GL\_LIGHT[0,1,2,3], this is a constant factor that attenuates the light source; attenuation factor = 1/(kc +kl*d + kq*d*d); (kc,kl,kq)=(1,0,0) means no attenuation; only used for lights, where last component of light position is 1\\ \hline
    diffuse &     float &     1 &     0.5 &     diffuse value of GL\_LIGHT[0,1,2,3]\\ \hline
    enable &     bool &     1 &     True &     turn on/off light\\ \hline
    lightRadius &     float &     1 &     0.1 &     only used by raytracers: radius of light used to compute smooth shadows (approximated by raytracer.lightRadiusVariations); if lightRadiusVariations>1, this value defines the radius of the light, converting point lights into distributed lights (slower)\\ \hline
    linearAttenuation &     float &     1 &     0.0 &     linear attenuation coefficient of GL\_LIGHT[0,1,2,3], this is a linear factor for attenuation of the light source with distance\\ \hline
    position &     Float4 &     4 &     [2.,2.,10.,0.] &     \tabnewline 4D position vector of GL\_LIGHT[0,1,2,3]; 4th value should be 0 for directional lights that are (almost) infinitely far away, like the sun, but 1 for position-based lights (and for attenuation factor being calculated); light0 is also used for shadows, so you need to adjust this position to be located at a reasonable location; the openGL renderer uses shadow volumes and approximates directional lights by enlarging the direction to 200 times maxSceneSize, while the raytracer uses the correct direction; see opengl manuals\\ \hline
    quadraticAttenuation &     float &     1 &     0.0 &     quadratic attenuation coefficient of GL\_LIGHT[0,1,2,3], this is a quadratic factor for attenuation of the light source with distance\\ \hline
    shadow &     UFloat &      &     0. &     in OpenGL renderer, the shadow parameter $\in [0 ... 1]$ prescribes amount of shadow of light [0,1,2,3] that is added to the scene, using light position (or only direction), accumulating for each light; if this parameter is different from 0, rendering of triangles becomes approx.\ 5 times more expensive, so take care in case of complex scenes; for complex object, such as spheres with fine resolution or for particle systems, the present approach has limitations and leads to artifacts and unrealistic shadows; for raytracer, shadow is included by a physics-based model for each light if shadow>0, accumulating effects of each light source\\ \hline
    specular &     float &     1 &     0.5 &     specular value of GL\_LIGHT[0,1,2,3]\\ \hline
    useCameraFrame &     bool &     1 &     False &     set False to set light positions and directions relative to model frame; True: lights are in camera frame, not following the visual transformations; this was True up to Exudyn 1.9.174\\ \hline
	  \end{longtable}
	\end{center}
...
\begin{center}
  ...
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    \hline
    \bf Name & \bf type / function return type & \bf size & \bf default value / function args & \bf description \\ \hline
    advanced &     VSettingsOpenGLAdvanced &      &      &     advanced settings for openGL\\ \hline
    light0 &     VSettingsLight &      &      &     settings for light0 and shadow\\ \hline
    light1 &     VSettingsLight &      &      &     settings for light1 and shadow\\ \hline
    light2 &     VSettingsLight &      &      &     settings for light2 and shadow\\ \hline
    light3 &     VSettingsLight &      &      &     settings for light3 and shadow\\ \hline
    ...
    lightModelAmbient &     Float4 &     4 &     [0.4,0.4,0.4,1.] &     \tabnewline global ambient light (needed for faces that are close to orthogonal to light or faces in shadow region); maps to OpenGL glLightModeli(GL\_LIGHT\_MODEL\_AMBIENT,[r,g,b,a]); also used by raytracer\\ \hline
    ...
    zMaxSceneFactor &     PFloat &      &     2. &     factor multiplied with maxSceneSize to avoid clipping of modelview; larger values reduce clipping of near or far objects, but may lead to artifacts (so-called Z-fighting)\\ \hline
	  \end{longtable}
	\end{center}
...
\mysubsubsubsection{VSettingsInteractiveAdvanced}
\label{sec:VSettingsInteractiveAdvanced}
Advanced settings for interactive.

\noindent VSettingsInteractiveAdvanced has the following items:
%reference manual TABLE
\begin{center}
  \footnotesize
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    ...
    selectionHighlights &     bool &      &     True &     True: enable mouse click to highlights item (default: red)\\ \hline
    selectionLeftMouse &     bool &      &     True &     True: enable left mouse click on items to show basic information\\ \hline
    ...
    selectionRightMouse &     bool &      &     True &     True: enable right mouse click on items to show dictionary (read only!)\\ \hline
...
\begin{center}
  ...
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    \hline
    \bf Name & \bf type / function return type & \bf size & \bf default value / function args & \bf description \\ \hline
    advanced &     VSettingsInteractiveAdvanced &      &      &     advanced interactive visualization settings\\ \hline
    openVR &     VSettingsOpenVR &      &      &     openVR visualization settings\\ \hline
    autoRotateModelView &     bool &      &     False &     True: rotate model view with autorotation\\ \hline
    autoRotationVelocity &     Float3 &     3 &     [0.,0.,1.047198] &     \tabnewline Angular velocity vector for auto-rotation of scene (only visualization view is rotated, not the model itself!)\\ \hline
    highlightItemIndex &     Int &      &     -1 &     index of item that shall be highlighted (e.g., to find item which cauess problems); if set -1, no item is highlighted\\ \hline
    highlightItemType &     ItemType &      &     ItemType::\_None &     item type (Node, Object, ...) that shall be highlighted (e.g., to find item which cauess problems)\\ \hline
    ...
    ignoreKeys &     bool &      &     False &     True: ignore keyboard input except escape and 'F2' keys; used for interactive mode, e.g., to perform kinematic analysis; This flag can be switched with key 'F2'; if ignoreKeys=True, then keyPressUserFunction can be used!\\ \hline
    ...
    logMouseCoordinates &     bool &      &     True &     True: if showMouseCoordinates=True, also log mouse coordinates (transformed to model coordinates); only works for axis-aligned ortho-projections and shows the coordinates of the current plane\\ \hline
...
\begin{center}
  ...
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    ...
    view0 &     VSettingsView &      &      &     Settings for main view 0\\ \hline
    view1 &     VSettingsView &      &      &     Settings for sub-view 1\\ \hline
    view2 &     VSettingsView &      &      &     Settings for sub-view 2\\ \hline
    view3 &     VSettingsView &      &      &     Settings for sub-view 3\\ \hline
	  \end{longtable}
	\end{center}

The following parameter changes have been made:
\bi
  \item general.drawCoordinateSystem $\ra$ view0.scene.drawCoordinateSystem (changed in version 1.10.80, expires: 2030)
  \item general.drawWorldBasis $\ra$ view0.scene.drawWorldBasis (changed in version 1.10.80, expires: 2030)
  \item general.showComputationInfo $\ra$ view0.window.showComputationInfo (changed in version 1.10.80, expires: 2030)
  \item general.textSize $\ra$ view0.window.globalFontSize (changed in version 1.10.80, expires: 2030)
  \item general.worldBasisSize $\ra$ view0.scene.worldBasisSize (changed in version 1.10.80, expires: 2030)
  \item contour.colorBarPrecision $\ra$ contour.advanced.colorBarPrecision (changed in version 1.10.80, expires: 2030)
  \item contour.colorBarTiling $\ra$ contour.advanced.colorBarTiling (changed in version 1.10.80, expires: 2030)
  \item contour.showColorBar $\ra$ contour.advanced.showColorBar (changed in version 1.10.80, expires: 2030)
  \item window.alwaysOnTop $\ra$ view0.window.alwaysOnTop (changed in version 1.10.80, expires: 2030)
  \item window.ignoreKeys $\ra$ interactive.ignoreKeys (changed in version 1.10.80, expires: 2030)
  \item window.keyPressUserFunction $\ra$ interactive.keyPressUserFunction (changed in version 1.10.80, expires: 2030)
  \item window.limitWindowToScreenSize $\ra$ general.limitWindowToScreenSize (changed in version 1.10.80, expires: 2030)
  \item window.maximize $\ra$ view0.window.maximize (changed in version 1.10.80, expires: 2030)
  \item window.reallyQuitTimeLimit $\ra$ general.reallyQuitTimeLimit (changed in version 1.10.80, expires: 2030)
  \item window.renderWindowSize $\ra$ view0.window.renderWindowSize (changed in version 1.10.80, expires: 2030)
  \item window.showMouseCoordinates $\ra$ view0.window.showMouseCoordinates (changed in version 1.10.80, expires: 2030)
  \item window.showRenderStateInfo $\ra$ view0.window.showRenderStateInfo (changed in version 1.10.80, expires: 2030)
  \item window.showWindow $\ra$ view0.window.showWindow (changed in version 1.10.80, expires: 2030)
  \item window.startupTimeout $\ra$ general.rendererStartupTimeout (changed in version 1.10.80, expires: 2030)
  \item raytracer.ambientLightColor $\ra$ openGL.lightModelAmbient (changed in version 1.10.80, expires: 2030)
  \item raytracer.backgroundColorReflections $\ra$ raytracer.advanced.backgroundColorReflections (changed in version 1.10.80, expires: 2030)
  \item raytracer.enable $\ra$ view0.camera.useRaytracer (changed in version 1.10.80, expires: 2030)
  \item raytracer.lightRadius $\ra$ openGL.light0.lightRadius (changed in version 1.10.80, expires: 2030)
  \item raytracer.searchTreeFactor $\ra$ raytracer.advanced.searchTreeFactor (changed in version 1.10.80, expires: 2030)
  \item raytracer.shadowScalingFactor $\ra$ raytracer.advanced.shadowScalingFactor (changed in version 1.10.80, expires: 2030)
  \item raytracer.shadowSmoothingSteps $\ra$ raytracer.advanced.shadowSmoothingSteps (changed in version 1.10.80, expires: 2030)
  \item raytracer.showText $\ra$ raytracer.advanced.showText (changed in version 1.10.80, expires: 2030)
  \item raytracer.tilesPerThread $\ra$ raytracer.advanced.tilesPerThread (changed in version 1.10.80, expires: 2030)
  \item raytracer.zBiasLines $\ra$ raytracer.advanced.zBiasLines (changed in version 1.10.80, expires: 2030)
  \item raytracer.zOffsetCamera $\ra$ openGL.dummy (changed in version 1.10.80, expires: 2030)
  \item openGL.clippingPlaneColor $\ra$ openGL.advanced.clippingPlaneColor (changed in version 1.10.80, expires: 2030)
  \item openGL.clippingPlaneDistance $\ra$ view0.camera.clippingPlaneDistance (changed in version 1.10.80, expires: 2030)
  \item openGL.clippingPlaneNormal $\ra$ view0.camera.clippingPlaneNormal (changed in version 1.10.80, expires: 2030)
  \item openGL.depthSorting $\ra$ openGL.advanced.depthSorting (changed in version 1.10.80, expires: 2030)
  \item openGL.enableLight0 $\ra$ openGL.light0.enable (changed in version 1.10.80, expires: 2030)
  \item openGL.enableLight1 $\ra$ openGL.light1.enable (changed in version 1.10.80, expires: 2030)
  \item openGL.enableLighting $\ra$ openGL.advanced.enableLighting (changed in version 1.10.80, expires: 2030)
  \item openGL.facesTransparent $\ra$ view0.scene.facesTransparent (changed in version 1.10.80, expires: 2030)
  \item openGL.initialCenterPoint $\ra$ openGL.advanced.initialCenterPoint (changed in version 1.10.80, expires: 2030)
  \item openGL.initialMaxSceneSize $\ra$ openGL.advanced.initialMaxSceneSize (changed in version 1.10.80, expires: 2030)
  \item openGL.initialModelRotation $\ra$ openGL.advanced.initialModelRotation (changed in version 1.10.80, expires: 2030)
  \item openGL.initialZoom $\ra$ openGL.advanced.initialZoom (changed in version 1.10.80, expires: 2030)
  \item openGL.light0ambient $\ra$ openGL.dummy (changed in version 1.10.80, expires: 2030)
  \item openGL.light0constantAttenuation $\ra$ openGL.light0.constantAttenuation (changed in version 1.10.80, expires: 2030)
  \item openGL.light0diffuse $\ra$ openGL.light0.diffuse (changed in version 1.10.80, expires: 2030)
  \item openGL.light0linearAttenuation $\ra$ openGL.light0.linearAttenuation (changed in version 1.10.80, expires: 2030)
  \item openGL.light0position $\ra$ openGL.light0.position (changed in version 1.10.80, expires: 2030)
  \item openGL.light0quadraticAttenuation $\ra$ openGL.light0.quadraticAttenuation (changed in version 1.10.80, expires: 2030)
  \item openGL.light0specular $\ra$ openGL.light0.specular (changed in version 1.10.80, expires: 2030)
  \item openGL.light1ambient $\ra$ openGL.dummy (changed in version 1.10.80, expires: 2030)
  \item openGL.light1constantAttenuation $\ra$ openGL.light1.constantAttenuation (changed in version 1.10.80, expires: 2030)
  \item openGL.light1diffuse $\ra$ openGL.light1.diffuse (changed in version 1.10.80, expires: 2030)
  \item openGL.light1linearAttenuation $\ra$ openGL.light1.linearAttenuation (changed in version 1.10.80, expires: 2030)
  \item openGL.light1position $\ra$ openGL.light1.position (changed in version 1.10.80, expires: 2030)
  \item openGL.light1quadraticAttenuation $\ra$ openGL.light1.quadraticAttenuation (changed in version 1.10.80, expires: 2030)
  \item openGL.light1specular $\ra$ openGL.light1.specular (changed in version 1.10.80, expires: 2030)
  \item openGL.lightModelLocalViewer $\ra$ openGL.advanced.lightModelLocalViewer (changed in version 1.10.80, expires: 2030)
  \item openGL.lightModelTwoSide $\ra$ openGL.advanced.lightModelTwoSide (changed in version 1.10.80, expires: 2030)
  \item openGL.lightPositionsInCameraFrame $\ra$ openGL.light0.useCameraFrame (changed in version 1.10.80, expires: 2030)
  \item openGL.lineSmooth $\ra$ openGL.advanced.lineSmooth (changed in version 1.10.80, expires: 2030)
  \item openGL.materialAmbientAndDiffuse $\ra$ openGL.materialSpecular (changed in version 1.10.80, expires: 2030)
  \item openGL.perspective $\ra$ view0.camera.perspective (changed in version 1.10.80, expires: 2030)
  \item openGL.polygonOffset $\ra$ openGL.advanced.polygonOffset (changed in version 1.10.80, expires: 2030)
  \item openGL.shadeModelSmooth $\ra$ openGL.advanced.shadeModelSmooth (changed in version 1.10.80, expires: 2030)
  \item openGL.shadow $\ra$ openGL.light0.shadow (changed in version 1.10.80, expires: 2030)
  \item openGL.shadowPolygonOffset $\ra$ openGL.advanced.shadowPolygonOffset (changed in version 1.10.80, expires: 2030)
  \item openGL.showFaceEdges $\ra$ view0.scene.showFaceEdges (changed in version 1.10.80, expires: 2030)
  \item openGL.showFaces $\ra$ view0.scene.showFaces (changed in version 1.10.80, expires: 2030)
  \item openGL.showLines $\ra$ view0.scene.showLines (changed in version 1.10.80, expires: 2030)
  \item openGL.showMeshEdges $\ra$ view0.scene.showMeshEdges (changed in version 1.10.80, expires: 2030)
  \item openGL.showMeshFaces $\ra$ view0.scene.showMeshFaces (changed in version 1.10.80, expires: 2030)
  \item openGL.textLineSmooth $\ra$ openGL.advanced.textLineSmooth (changed in version 1.10.80, expires: 2030)
  \item openGL.textLineWidth $\ra$ openGL.advanced.textLineWidth (changed in version 1.10.80, expires: 2030)
  \item interactive.highlightColor $\ra$ interactive.advanced.highlightColor (changed in version 1.10.80, expires: 2030)
  \item interactive.highlightOtherColor $\ra$ interactive.advanced.highlightOtherColor (changed in version 1.10.80, expires: 2030)
  \item interactive.joystickScaleRotation $\ra$ interactive.advanced.joystickScaleRotation (changed in version 1.10.80, expires: 2030)
  \item interactive.joystickScaleTranslation $\ra$ interactive.advanced.joystickScaleTranslation (changed in version 1.10.80, expires: 2030)
  \item interactive.keypressRotationStep $\ra$ interactive.advanced.keypressRotationStep (changed in version 1.10.80, expires: 2030)
  \item interactive.keypressTranslationStep $\ra$ interactive.advanced.keypressTranslationStep (changed in version 1.10.80, expires: 2030)
  \item interactive.lockModelView $\ra$ view0.window.lockModelView (changed in version 1.10.80, expires: 2030)
  \item interactive.mouseMoveRotationFactor $\ra$ interactive.advanced.mouseMoveRotationFactor (changed in version 1.10.80, expires: 2030)
  \item interactive.pauseWithSpacebar $\ra$ interactive.advanced.pauseWithSpacebar (changed in version 1.10.80, expires: 2030)
  \item interactive.selectionHighlights $\ra$ interactive.advanced.selectionHighlights (changed in version 1.10.80, expires: 2030)
  \item interactive.selectionLeftMouse $\ra$ interactive.advanced.selectionLeftMouse (changed in version 1.10.80, expires: 2030)
  \item interactive.selectionLeftMouseItemTypes $\ra$ interactive.advanced.selectionLeftMouseItemTypes (changed in version 1.10.80, expires: 2030)
  \item interactive.selectionRightMouse $\ra$ interactive.advanced.selectionRightMouse (changed in version 1.10.80, expires: 2030)
  \item interactive.selectionRightMouseGraphicsData $\ra$ interactive.advanced.selectionRightMouseGraphicsData (changed in version 1.10.80, expires: 2030)
  \item interactive.trackMarker $\ra$ view0.camera.trackMarker (changed in version 1.10.80, expires: 2030)
  \item interactive.trackMarkerMbsNumber $\ra$ view0.camera.trackMarkerMbsNumber (changed in version 1.10.80, expires: 2030)
  \item interactive.trackMarkerOrientation $\ra$ view0.camera.trackMarkerOrientation (changed in version 1.10.80, expires: 2030)
  \item interactive.trackMarkerPosition $\ra$ view0.camera.trackMarkerPosition (changed in version 1.10.80, expires: 2030)
  \item interactive.zoomStepFactor $\ra$ interactive.advanced.zoomStepFactor (changed in version 1.10.80, expires: 2030)
\ei
...
\begin{center}
  ...
  \begin{longtable}{| p{4.2cm} | p{2.5cm} | p{0.3cm} | p{3.0cm} | p{6cm} |}
    ...
    cpuSolverStartTime &     Real &      &     0. &     CPU start time of main solver (not including initial conditions); cpuSolverStartTime-cpuStartTime gives time for initialization\\ \hline
    ...
    simulationStoppedByUser &     bool &      &     False &     flag (initialized false) is set true when user stops the simulation (press Q, Escape, etc.)\\ \hline
    simulationStoppedByUserFunction &     \tabnewline bool &      &     False &     flag (initialized false) is set true when a user function (PreStep, PostNewton, etc.) sends termination signal\\ \hline
    simulationTimeout &     bool &      &     False &     flag (initialized false) is set true when exudyn.special.solver.timeout is reached (and timeout is >= 0)\\ \hline
