...
A 3D point node for point masses or solid finite elements which has 3 displacement degrees of freedom for \hac{ODE2}.
\vspace{12pt}\\
...
A 2D point node for point masses or solid finite elements which has 2 displacement degrees of freedom for \ac{ODE2}.
\vspace{12pt}\\
...
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/ANCFslidingJoint.py}{\texttt{ANCFslidingJoint.py}} (Examples/)
...
A 2D rigid body node for rigid bodies or beams. The node has 2 displacement degrees of freedom and one rotation coordinate (rotation around z-axis: $\psi_0$). All coordinates are \hac{ODE2}, used for second order differetial equations.
\vspace{12pt}\\
...
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/TestModels/ANCFCableBeamDampingTest.py}{\texttt{ANCFCableBeamDampingTest.py}} (TestModels/)
...
A node containing a number of \hac{ODE2} variables. Use this node e.g. for scalar dynamic equations (Mass1D), for ObjectGenericODE2 or for the Eulerian coordinate in the ALECable element. NOTE: referenceCoordinates and all initialCoordinates(\_t) must be initialized, because no default values exist.
\vspace{12pt}\\
...
A node containing a number of \hac{ODE1} variables. Use this node e.g. for linear state space systems. NOTE: referenceCoordinates and initialCoordinates must be initialized, because no default values exist.
\vspace{12pt}\\
\vspace{12pt} \noindent 
...
A node containing a number of \hac{AE} variables. Use e.g. linear state space systems. NOTE: referenceCoordinates and initialCoordinates must be initialized, because no default values exist.
\vspace{12pt}\\
\vspace{12pt} \noindent 
...
\begin{center}
  ...
  \begin{longtable}{| p{4.5cm} | p{2.5cm} | p{0.5cm} | p{2.5cm} | p{6cm} |}
    ...
    crossSectionPenaltyFactor &     Vector3D &      &     [1.,1.,1.] &     \tabnewline  [SI:1] additional penalty factors for cross section deformation, which are in total $k_{cs} = [f_{yy}\cdot EA,\, f_{zz}\cdot EA,\, f_{yz}\cdot (GA_y+GA_z)]\tp$\\ \hline
    crossSectionDamping &     Vector3D &      &     [0.,0.,0.] &     \tabnewline  [SI:1] viscous damping according to penalty factors for cross section deformation; the damping is relative to the stiffness and should be thus usually much smaller than 1; the viscous damping factors read  $d_{cs} = [d_{fyy}\cdot EA,\, d_{fzz}\cdot EA,\, d_{fyz}\cdot (GA_y+GA_z)]\tp$\\ \hline
...
\startTable{input parameter}{symbol}{description see tables above}
\rowTable{physicsLength}{$L$}{}
\rowTable{crossSectionPenaltyFactor}{$k_{cs} = [f_{yy},\,f_{zz},\,f_{yz}]\tp$}{}
\rowTable{crossSectionDamping}{$d_{cs} = [d_{fyy},\,d_{fzz},\,d_{fyz}]\tp$}{}
\finishTable
...
\begin{center}
  ...
  \begin{longtable}{| p{4.5cm} | p{2.5cm} | p{0.5cm} | p{2.5cm} | p{6cm} |}
    ...
    \bf Name & \bf type & \bf size & \bf default value & \bf description \\ \hline
    name &     String &      &     '' &     objects's unique name\\ \hline
    nodeNumbers &     ArrayNodeIndex &      &     [] &     two node numbers for beam element\\ \hline
    physicsLength &     UReal &      &     0. &      [SI:m] reference length of beam; such that the total volume (e.g. for volume load) gives $\rho A L$; must be positive\\ \hline
    physicsMassPerLength &     UReal &      &     0. &      [SI:kg/m] mass per length of beam\\ \hline
...
\begin{center}
  ...
  \begin{longtable}{| p{4.5cm} | p{2.5cm} | p{0.5cm} | p{2.5cm} | p{6cm} |}
    ...
    \bf Name & \bf type & \bf size & \bf default value & \bf description \\ \hline
    name &     String &      &     '' &     objects's unique name\\ \hline
    physicsThickness &     NumpyVector &      &     [] &      [SI:m] thickness of plate either provided as scalar or as vector (4 values, same order as local element node numbers) values that are linearly interpolated from nodal values; dimensionality must agree between thickness, strainCoefficients and curvatureCoefficients\\ \hline
    physicsDensity &     UReal &      &     0. &      [SI:kg/m$^3$] density of the plate, possibly averaged over thickness\\ \hline
    physicsMassProportionalDamping &     Real &      &     0. &     mass-proportional damping coefficient $\alpha$ [SI:1/s]; adds massmatrix proportional damping forces $\fv_d = \alpha \Mm \dot{\qv}$\\ \hline
    ...
    slopesScalingX &     Vector4D &     4 &     [-1.,-1.,-1.,-1.] &     \tabnewline scaling of x-slopes at each element node; flat elements: half of the side length of the element; curved: optimal values such that curved geometry is best approximated; if negative (default) values are used, length is computed from node distances.\\ \hline
    slopesScalingY &     Vector4D &     4 &     [-1.,-1.,-1.,-1.] &     \tabnewline scaling of y-slopes at each element node; flat elements: half of the side length of the element; curved: optimal values such that curved geometry is best approximated; if negative (default) values are used, length is computed from node distances.\\ \hline
    nodeNumbers &     NodeIndex4 &      &     [invalid [-1], invalid [-1], invalid [-1], invalid [-1]] &     \tabnewline 4 NodePointSlope12 node numbers, with local (xi,eta) coordinates as [(-1,-1),(1,-1),(1,1),(-1,1)]\\ \hline
...
\hline
\bf output variable & \bf symbol & \bf description \\ \hline
Position & $\LU{0}{\pv\cConfig(x,y,z)}$ & global position vector of local position $[x,y,z]$\\ \hline
Displacement & $\LU{0}{\uv\cConfig(x,y,z)} = \LU{0}{\pv\cConfig(x,y,z)} - \LU{0}{\pv\cRef(x,y,z)}$ & global displacement vector of local position\\ \hline
Velocity & $\LU{0}{\vv(x,y,z)} = \LU{0}{\dot \rv(x,y,z)}$ & global velocity vector of local position\\ \hline
Director1 & $\rv_x(x,y,z)$ & (axial) slope vector of local position (at $z$=0)\\ \hline
Director2 & $\rv_y(x,y,z)$ & (axial) slope vector of local position (at $z$=0)\\ \hline
StrainLocal & $\varepsilon$ & axial strain (scalar) of local axis position (at Z=0)\\ \hline
CurvatureLocal & $[K_x, K_y, K_z]\tp$ & local curvature vector\\ \hline
ForceLocal & $N$ &  (local) section normal force per length (scalar, including reference strains) (at $z$=0)\\ \hline
TorqueLocal & $M$ &  (local) bending moment per length (scalar) (at $z$=0), which are bending moments as there is no torque\\ \hline
StressLocal &  & local inplane stress components\\ \hline
Acceleration & $\LU{0}{\av(x,y,z)} = \LU{0}{\ddot \rv(x,y,z)}$ & global acceleration vector of local position\\ \hline
\end{longtable}
\end{center}
 \noindent
...
\bi
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/bungeeJump.py}{\texttt{bungeeJump.py}} (Examples/)
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/NGsolveFFRFSlidingJoint.py}{\texttt{NGsolveFFRFSlidingJoint.py}} (Examples/)
...
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/pistonEngine.py}{\texttt{pistonEngine.py}} (Examples/)
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/pymeshlabFileImport.py}{\texttt{pymeshlabFileImport.py}} (Examples/)
...
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/NGsolveCreateFFRFreducedOrder.py}{\texttt{NGsolveCreateFFRFreducedOrder.py}} (Examples/)
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/bungeeJump.py}{\texttt{bungeeJump.py}} (Examples/)
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/chainDriveExample.py}{\texttt{chainDriveExample.py}} (Examples/)
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/humanRobotInteraction.py}{\texttt{humanRobotInteraction.py}} (Examples/)
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/NGsolveFFRFSlidingJoint.py}{\texttt{NGsolveFFRFSlidingJoint.py}} (Examples/)
...
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/sliderCrank3DwithANCFbeltDrive.py}{\texttt{sliderCrank3DwithANCFbeltDrive.py}} (Examples/)
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/TestModels/ACFtest.py}{\texttt{ACFtest.py}} (TestModels/)
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/TestModels/ANCFThinPlateTests.py}{\texttt{ANCFThinPlateTests.py}} (TestModels/)
...
\mysubsubsection{ObjectJointSliding}
\label{sec:item:ObjectJointSliding}
...
\noindent \mybold{Additional information for ObjectJointSliding}:
\bi
  \item This \texttt{Object} has/provides the following types = \texttt{Connector}, \texttt{Constraint}
  \item Requested \texttt{Marker} type = \texttt{\_None}
  \item Requested \texttt{Node} type = \texttt{GenericData}
  \item {\bf Short name} for Python = \texttt{SlidingJoint}
  \item {\bf Short name} for Python visualization object = \texttt{VSlidingJoint}
\ei\vspace{12pt} \noindent 
The item \mybold{ObjectJointSliding} with type = 'JointSliding' has the following parameters:
\vspace{-0.5cm}\\
\vspace{-0.5cm}\\
%reference manual TABLE
\begin{center}
  ...
  \begin{longtable}{| p{4.5cm} | p{2.5cm} | p{0.5cm} | p{2.5cm} | p{6cm} |}
    ...
    constrainRotations &     ArrayIndex &     3 &     [1,1,1] &     \tabnewline flags for constrained rotation about x, y and z-axis: if flag=1, add constraint on rotation of marker m0 relative to respective axis; flag=0: sliding body can rotate freely about this axis; for ANCFCable, rotation about x-axis cannot be constrained\\ \hline
    constrainTranslations &     ArrayIndex &     3 &     [1,1,1] &     \tabnewline flags for constrained translation in x, y and z-direction: if flag=1, add constraint on translation of marker m0 relative to respective axis; flag=0: sliding body can translate freely about this axis; along x-axis this should be usually 0, except for driven motion\\ \hline
    ...
    activeConnector &     Bool &      &     True &     flag, which determines, if the connector is active; used to deactivate (temporarily) a connector or constraint\\ \hline
    visualization &     VObjectJointSliding &      &      &     parameters for visualization of item\\ \hline
\end{longtable}
\end{center}

\noindent The item VObjectJointSliding has the following parameters:
%reference manual TABLE
\begin{center}
  \footnotesize
  \begin{longtable}{| p{4.5cm} | p{2.5cm} | p{0.5cm} | p{2.5cm} | p{6cm} |}
...
\end{center}
\par\noindent\rule{\textwidth}{0.4pt}
\mysubsubsubsection{DESCRIPTION of ObjectJointSliding:}
\label{description_ObjectJointSliding}
\paragraph{Information on input parameters:} 
\startTable{input parameter}{symbol}{description see tables above}
\rowTable{markerNumbers}{$[m0,m1]\tp$}{}
...
\vspace{6pt}\par\noindent\rule{\textwidth}{0.4pt}
%
\noindent For examples on ObjectJointSliding see Relevant Examples and TestModels with weblink:
\bi
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/ANCFslidingJoint.py}{\texttt{ANCFslidingJoint.py}} (Examples/)
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/NGsolveFFRFSlidingJoint.py}{\texttt{NGsolveFFRFSlidingJoint.py}} (Examples/)
...
A node-Marker attached to a \hac{ODE2} coordinate of a node; this marker allows to connect a coordinate-based constraint or connector to a nodal coordinate (also NodeGround); for \hac{ODE1} coordinates use \texttt{MarkerNodeODE1Coordinate}.
\vspace{12pt}\\
...
A node-Marker attached to all \hac{ODE2} coordinates of a node. IN CONTRAST to MarkerNodeCoordinate, the marker coordinates INCLUDE the reference values! For \hac{ODE1} coordinates use \texttt{MarkerNodeODE1Coordinates}.
\vspace{12pt}\\
...
\mysubsubsection{MarkerBodyBeamShape}
\label{sec:item:MarkerBodyBeamShape}
A special Marker attached to a 3D beam finite element which provides at least position and tangent to the beam axis.
\vspace{12pt}\\

\noindent \mybold{Additional information for MarkerBodyBeamShape}:
\bi
  \item This \texttt{Marker} has/provides the following types = \texttt{Object}, \texttt{Body}
\ei\vspace{12pt} \noindent 
The item \mybold{MarkerBodyBeamShape} with type = 'BodyBeamShape' has the following parameters:
\vspace{-0.5cm}\\
\vspace{-0.5cm}\\
%reference manual TABLE
\begin{center}
  ...
  \begin{longtable}{| p{4.5cm} | p{2.5cm} | p{0.5cm} | p{2.5cm} | p{6cm} |}
    ...
    \bf Name & \bf type & \bf size & \bf default value & \bf description \\ \hline
    name &     String &      &     '' &     marker's unique name\\ \hline
    bodyNumber &     ObjectIndex &      &     invalid (-1) &     \tabnewline body number to which marker is attached to (beam type)\\ \hline
    visualization &     VMarkerBodyBeamShape &      &      &     parameters for visualization of item\\ \hline
\end{longtable}
\end{center}

\noindent The item VMarkerBodyBeamShape has the following parameters:
%reference manual TABLE
\begin{center}
  \footnotesize
  \begin{longtable}{| p{4.5cm} | p{2.5cm} | p{0.5cm} | p{2.5cm} | p{6cm} |}
...
\end{center}
\par\noindent\rule{\textwidth}{0.4pt}
\mysubsubsubsection{DESCRIPTION of MarkerBodyBeamShape:}
\label{description_MarkerBodyBeamShape}
\vspace{6pt}\par\noindent\rule{\textwidth}{0.4pt}
%
\noindent For examples on MarkerBodyBeamShape see Relevant Examples and TestModels with weblink:
\bi
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/ANCFslidingJoint.py}{\texttt{ANCFslidingJoint.py}} (Examples/)
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/NGsolveFFRFSlidingJoint.py}{\texttt{NGsolveFFRFSlidingJoint.py}} (Examples/)
