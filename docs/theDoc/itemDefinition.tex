...
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/sliderCrankCMSacme.py}{\texttt{sliderCrankCMSacme.py}} (Examples/)
...
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/geneticOptimizationExample.py}{\texttt{geneticOptimizationExample.py}} (Examples/)
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/Examples/geneticOptimizationSliderCrank.py}{\texttt{geneticOptimizationSliderCrank.py}} (Examples/)
...
{\bf The following output variables are available as OutputVariableType in sensors, Get...Output() and other functions}: 
\startTable{output variable}{symbol}{description}
\rowTable{Position}{$\LU{0}{\pv}\cConfig(\pLocB) = \pRefG\cConfig + \pRefG\cRef + \LU{0b}{\Rot}\pLocB$}{global position vector of body-fixed point given by local position vector}
\rowTable{Displacement}{$\LU{0}{\uv}\cConfig + \LU{0b}{\Rot}\pLocB$}{global displacement vector of body-fixed point given by local position vector}
\rowTable{Velocity}{$\LU{0}{\vv}\cConfig(\pLocB) = \LU{0}{\dot\uv}\cConfig + \LU{0b}{\Rot}(\LU{b}{\tomega} \times \pLocB\cConfig)$}{global velocity vector of body-fixed point given by local position vector}
\rowTable{VelocityLocal}{$\LU{b}{\vv}\cConfig(\pLocB) = \LU{b0}{\Rot} \LU{0}{\vv}\cConfig(\pLocB)$}{local (body-fixed) velocity vector of body-fixed point given by local position vector}
...
\rowTable{Acceleration}{$\LU{0}{\av}\cConfig(\pLocB) = \LU{0}{\ddot\uv} + \LU{0}{\talpha} \times (\LU{0b}{\Rot} \pLocB) +  \LU{0}{\tomega} \times ( \LU{0}{\tomega} \times(\LU{0b}{\Rot} \pLocB))$}{global acceleration vector of body-fixed point given by local position vector}
\rowTable{AngularAcceleration}{$\LU{0}{\talpha}\cConfig$}{angular acceleration vector of body}
\finishTable
 \noindent
    \mysubsubsubsection{Definition of quantities}
...
\finishTable
 \noindent
    ...
    The (internal) Jacobian $\Jm$ of \label{eq_ObjectGenericODE2_EOM} (assuming $\fv$ to be constant!) reads
    \be
    ...
    In case that a \texttt{jacobianUserFunction} is specified, it must represent the jacobian of the \ac{LHS} of \label{eq_ObjectGenericODE2_EOM} 
...
\finishTable
 \noindent
    ...
    \be
      \fv_{SD} = \mathrm{UF}(mbs, t, i_N, \Delta\! \LU{0}{\pv}, \Delta\! \LU{0}{\vv}, \kv, \dv, \vv_{\mathrm{off}})
    \ee
    and \texttt{iN} represents the itemNumber (=objectNumber).
...
%definition how output variables are computed:
    ...
    If \texttt{activeConnector = True}, the vector spring force is computed as
    \be
      \tau_{SD} = k \left(\Delta\theta - v_\mathrm{off} \right) + d \,\Delta\omega + \tau_c
    \ee
...
{\bf The following output variables are available as OutputVariableType in sensors, Get...Output() and other functions}: 
\startTable{output variable}{symbol}{description}
\rowTable{Position}{$\LU{0}{\pv}_{G}$}{current global position of contact point between roller and ground}
\rowTable{Velocity}{$\LU{0}{\vv}_{G}$}{current velocity of the trail (contact) point in global coordinates; this is the velocity with which the contact moves over the ground plane}
...
\begin{center}
  ...
  \begin{longtable}{| p{4.5cm} | p{2.5cm} | p{0.5cm} | p{2.5cm} | p{6cm} |}
    ...
    name &     String &      &     '' &     connector's unique name\\ \hline
    markerNumbers &     ArrayMarkerIndex &     \tabnewline  &     [ MAXINT, MAXINT ] &     \tabnewline markers define contact gap\\ \hline
    nodeNumber &     NodeIndex &      &     MAXINT &     \tabnewline node number of a NodeGenericData with 3 $\times n_{cs}$  dataCoordinates (used for active set strategy $\ra$ hold the gap of the last discontinuous iteration and the friction state)\\ \hline
...
A node-Marker attached to all \hac{ODE2} coordinates of a node; IN CONTRAST DOT MarkerNodeCoordinate, the marker coordinates INCLUDE the reference values!; this marker allows connecting a coordinate-based constraint or connector to a nodal coordinate (also NodeGround); for \hac{ODE1} coordinates use MarkerNodeODE1Coordinates (under development).\vspace{12pt}
...
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/TestModels/modelUnitTests.py}{\texttt{modelUnitTests.py}} (TestModels/)
\item \exuUrl{https://github.com/jgerstmayr/EXUDYN/blob/master/main/pythonDev/TestModels/perfSpringDamperExplicit.py}{\texttt{perfSpringDamperExplicit.py}} (TestModels/)
\item  ...
