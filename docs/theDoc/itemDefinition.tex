...
\bi
  ...
  \item {\bf Short name} for Python = {\bf Point}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf Point2D}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf RigidEP}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf RigidRxyz}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf RigidRotVecLG}\ei
...
\bi
  \item The Node has the following types = \texttt{Position2D}, \texttt{Orientation2D}, \texttt{Position}, \texttt{Orientation}
  \item {\bf Short name} for Python = {\bf Rigid2D}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf Point2DS1}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf PointGround}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf MassPoint}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf MassPoint2D}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf RigidBody}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf RigidBody2D}\ei
...
\begin{center}
  ...
  \begin{longtable}{| p{4.5cm} | p{2.5cm} | p{0.5cm} | p{2.5cm} | p{6cm} |}
    ...
    useFirstNodeAsReferenceFrame &     bool &      &     False &     set true, if first node ($n_0$) is used as floating reference frame; all other nodes are interpreted relative to the reference frame; used to implement FFRF (floating frame of reference formulation); NOTE that in this case, nodes $[n_1,\,\ldots,\,n_n]\tp$ are still drawn without the reference frame\\ \hline
...
This object is used to represent equations modelled by the floating frame of reference formulation (FFRF). It contains a RigidBodyNode (always node 0) and a list of other nodes representing the finite element nodes used in the FFRF.\vspace{12pt}
...
{\bf Description of Item}:
 \noindent
    ...
    {\bf Equations of motion}, in case that \texttt{computeFFRFterms = True} (NEEDS TO BE UPDATED):
    \be
      \left(\Mm_{user}(t,\qv,\dot \qv) + \mr{\Mm_{tt}}{\Mm_{tr}}{\Mm_{tf}} {}{\Mm_{rr}}{\Mm_{rf}} {\mathrm{sym.}}{}{\Mm_{ff}} \right) \ddot \qv + 
            \mr{0}{0}{0} {0}{0}{0} {0}{0}{\Dm_{ff}} \dot \qv + \mr{0}{0}{0} {0}{0}{0} {0}{0}{\Km_{ff}} \qv = 
            \fv_Q(\qv,\dot \qv) + \vp{\fv_r}{\LURU{0b}{\Am}{bd}{\mathrm{T}} \fv_{ff}} + \fv_{user}(t,\qv,\dot \qv)
    \ee
    ...
    Note that the user functions $\fv_{user}(t,\qv,\dot \qv)$ and $\Mm_{user}(t,\qv,\dot \qv)$ may be empty (=0). 
...
{\bf Description of Item}:
 \noindent
    ...
    \be
      \left(\Mm_{user}(t,\qv,\dot \qv) + \mr{\Mm_{tt}}{\Mm_{tr}}{\Mm_{tf}} {}{\Mm_{rr}}{\Mm_{rf}} {\mathrm{sym.}}{}{\Mm_{ff}} \right) \ddot \qv + 
            \mr{0}{0}{0} {0}{0}{0} {0}{0}{\Dm_{ff}} \dot \qv + \mr{0}{0}{0} {0}{0}{0} {0}{0}{\Km_{ff}} \qv = 
            \fv_Q(\qv,\dot \qv) + \fv_{user}(t,\qv,\dot \qv)
    \ee
    In case that \texttt{computeFFRFterms = False}, the mass terms $\Mm_{tt} \ldots \Mm_{ff}$ are zero (not computed) and
    the quadratic velocity vector $\fv_Q = \Null$.
    Note that the user functions $\fv_{user}(t,\qv,\dot \qv)$ and $\Mm_{user}(t,\qv,\dot \qv)$ may be empty (=0). 
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf Cable2D}\ei
...
\bi
  \item Requested node type: read detailed information of item
  \item {\bf Short name} for Python = {\bf ALECable2D}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf SpringDamper}\ei
...
\bi
  \item The Object has the following types = \texttt{Connector}
  \item Requested marker type = \texttt{Position}
  \item {\bf Short name} for Python = {\bf CartesianSpringDamper}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf RigidBodySpringDamper}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf CoordinateSpringDamper}\ei
...
\bi
  ...
  \item Requested marker type = \texttt{Position}
  \item {\bf Short name} for Python = {\bf DistanceConstraint}\ei
...
\bi
  \item The Object has the following types = \texttt{Connector}, \texttt{Constraint}
  \item Requested marker type = \texttt{Coordinate}
  \item {\bf Short name} for Python = {\bf CoordinateConstraint}\ei
...
\bi
  \item The Object has the following types = \texttt{Connector}, \texttt{Constraint}
  \item Requested marker type = \texttt{Position} + \texttt{Orientation}
  \item {\bf Short name} for Python = {\bf GenericJoint}\ei
...
\bi
  \item The Object has the following types = \texttt{Connector}, \texttt{Constraint}
  \item Requested marker type = \texttt{Position}
  \item {\bf Short name} for Python = {\bf SphericalJoint}\ei
...
\bi
  \item The Object has the following types = \texttt{Connector}, \texttt{Constraint}
  \item Requested marker type = \texttt{Position}
  \item {\bf Short name} for Python = {\bf RevoluteJoint2D}\ei
...
\bi
  \item The Object has the following types = \texttt{Connector}, \texttt{Constraint}
  \item Requested marker type = \texttt{Position} + \texttt{Orientation}
  \item {\bf Short name} for Python = {\bf PrismaticJoint2D}\ei
...
\bi
  ...
  \item Requested marker type = \texttt{\_None}
  \item Requested node type = \texttt{GenericData}
  \item {\bf Short name} for Python = {\bf SlidingJoint2D}\ei
...
\bi
  ...
  \item Requested marker type = \texttt{\_None}
  \item Requested node type: read detailed information of item
  \item {\bf Short name} for Python = {\bf ALEMovingJoint2D}\ei
...
\begin{center}
  ...
  \begin{longtable}{| p{4.5cm} | p{2.5cm} | p{0.5cm} | p{2.5cm} | p{6cm} |}
    ...
    name &     String &      &     '' &     marker's unique name\\ \hline
    bodyNumber &     Index &      &     MAXINT &     body number to which marker is attached to\\ \hline
    meshNodeNumbers &     ArrayIndex &      &     [] &     mesh node numbers of superelement which are used to compute the body-fixed marker position; the related nodes must provide 3D position information, such as NodePoint, NodePoint2D, NodeRigidBody[..]; in order to retrieve the global node number, the generic body needs to convert local into global node numbers\\ \hline
...
\bi
  \item Requested marker type = \texttt{Position}
  \item {\bf Short name} for Python = {\bf Force}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf Torque}\ei
...
\bi
  ...
  \item {\bf Short name} for Python = {\bf Gravity}\ei
