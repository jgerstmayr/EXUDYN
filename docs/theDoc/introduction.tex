...
\bi
  \item Python parts:
	...
	\ei
  \item C++ parts, see \fig{fig_exudyn_overview}:
	\bi
	  \item \texttt{exudyn}: on this level, there are just very few functions: SystemContainer(), StartRenderer(), StopRenderer()
	  ...
	  \item \texttt{mbs}: system created with \texttt{mbs = SC.AddSystem()}, this structure contains everything that defines a solvable multibody system; a large set of nodes, objects, markers, 
		...
		\item \texttt{mbs.systemData}: contains the initial, current, visualization, ... states of the system and holds the items, see \fig{fig_systemdata}
	\ei
\ei
%
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
\mysubsection{Items: Nodes, Objects, Loads, Markers, ...}
%
...
The simulation settings consists of a couple of substructures, e.g., for \texttt{solutionSettings}, \texttt{staticSolver}, \texttt{timeIntegration} as well as a couple of general options -- for details see Sections \ref{sec_solution_settings} -- \ref{sec_simulation_settings}.
...
Details on visualization settings and its substructures are provided in Sections \ref{sec_visualization_general} -- \ref{sec_visualization_settings}.
