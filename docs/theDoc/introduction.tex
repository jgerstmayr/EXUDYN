...
Currently, the \codeName\ module structure is split into a C++ core part and a set of Python parts,
see Figs.\ \ref{fig_exudyn_CandPython}.
\bi
  \item \mybold{C++ parts}, see Figs.\ \ref{fig_exudyn_CPP} and \ref{fig_system_overview}:
  \bi
    \item[--] \texttt{exudyn}:
...
\ignoreRST{The visualization settings dialog is shown exemplarily in \fig{fig_visualizationSettings}.}
...
The appearance of settings dialogs may be adjusted by \texttt{exudyn.GUI} variables:
\bi
  \item \texttt{exudyn.GUI.useRenderWindowDisplayScaling}: if True, the scaling will follow the current scaling of the render window; if False, it will use the \texttt{tkinter} internal scaling, which uses the main screen where the dialog is created (which won't scale well, if the window is moved to another screen).
  \item \texttt{exudyn.GUI.treeviewDefaultFontSize}: this is the base font size of the dialog (also right-mouse-button dialog)
  \item \texttt{exudyn.GUI.useRenderWindowDisplayScaling}: this factor is used to increase height of lines as compared to font size
  \item \texttt{exudyn.GUI.treeEditDefaultWidth}: unscaled width of, e.g., visualizationSettings
  \item \texttt{exudyn.GUI.treeEditDefaultHeight}: unscaled height of, e.g., visualizationSettings
  \item \texttt{exudyn.GUI.dialogDefaultWidth}: unscaled width of, e.g., right-mouse-button dialog
  \item \texttt{exudyn.GUI.treeEditDefaultHeight}: unscaled height of, e.g., right-mouse-button dialog
\ei
...
\begin{figure}[tbh]%
\begin{center}
\includegraphics[width=0.7\columnwidth]{figures/visualizationSettings}
\end{center}
\caption{View of visualization settings (press 'V' in render window to open dialog).}%
...
\mysubsubsection{Renderer and 3D graphics}
...
\mysubsubsection{Color, RGBA and alpha-transparency}
...
\bi
%deprecated: \item \texttt{utilities.AnimateSolution}, see Section \ref{sec:utilities:AnimateSolution}
...
import exudyn as exu
\end{lstlisting}
The faster versions are only pre-compiled for some Python versions, which can be determined by trying \texttt{import exudyn.exudynCPPfast}.
