...
The \mybold{simplest way to store the model view} is to \mybold{press CTRL-F3} when the renderer is running, to get the code for setting the model view printed to the console, e.g., 
\bi
\item[] \texttt{Set current view: SC.renderer.SetModelView(zoom=8.8,rotationVector=}\\
        \texttt{[-0.8120557,0.4727261,0.7176849],centerPoint=[1.562,-1.526,0])}
\ei
Then, just copy the code after \texttt{SC.renderer.Start}, see the following code snippet:
\pythonstyle\begin{lstlisting}
...
If you are using an interactive Python, there is a automated way to store and restore the current view (zoom, centerpoint, orientation, etc.) by using \texttt{SC.renderer.GetState()} and \texttt{SC.renderer.SetState()},
...
\horizontalRuler \\
%
Whenever \texttt{SC.renderer.Start()} is called, the renderState is reset (because it is assumed that the model has been changed and the previous view is invalid). However, you always can store and restore the renderstate manually.
Since version 1.10.98, the \texttt{ZoomAll} and \texttt{SetModelView} also work without starting the renderer (using only the raytracer). However, note that \texttt{ZoomAll} and \texttt{SetModelView} have to be called before the raytracer call RedrawAndGetImage(True) or after renderer.Start() using regular OpenGL.
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\mysubsubsectionlabel{Storing images and generating animations}{sec:overview:basics:animations}
%
...
Images can be stored conveniently either in the way shown below for series of images, or using the SolutionViewer, \refSection{sec:overview:basics:solutionviewer}.
For single images, you can use 
\bi
  \item \texttt{SC.renderer.RedrawAndGetImage()}
\ei
...
\mysubsubsubsection{Software rendering}
Setting the flag \texttt{useRaytracer=True} in \texttt{RedrawAndGetImage}, the software raytracer will be used -- see \refSection{sec:overview:basics:raytracing} for more details.
If the renderer has not yet been started, you ONLY can use the raytracer for image retrieval, however, you should use \texttt{renderer.ZoomAll} and \texttt{renderer.SetModelView} to adjust the view previously.
...
Retrieved images can be conveniently used with \texttt{matplotlib} for further manipulation or storing, also see examples:
%
\pythonstyle\begin{lstlisting}
...
\mysubsubsubsection{Generating Animations}
